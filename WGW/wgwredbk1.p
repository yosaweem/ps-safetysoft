...
DO:
 ...
 /* nv_class   = "120" .       */  /*    nv_class   = "520" .*/      
 /* nv_caryear = 2000 .        */  /*    nv_caryear = 2012 . */      
 /* nv_engcc   = 2959.         */  /*    nv_engcc   = 0 .    */      
 /* nv_ton     = 0 .           */  /*    nv_ton     = 8 .    */    
 /* nv_makdes = "ALFA ROMEO" . */  /*    nv_makdes  = "RCK" .*/    
 /* nv_moddes = " "  .         */  /*     nv_moddes  = "" .  */ 
 /* nv_si     = 0.             */  /*   nv_si      = 0.      */    
  IF nv_engcc <> 0  THEN DO:
       ASSIGN 
        ...
        IF nv_ton <> 0 THEN DO:
            IF nv_ton <= 4 THEN DO:
                FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                        ...
                        maktab_fil.tons    <= 4   NO-LOCK BY maktab_fil.si :
                ...
                IF rm_modcod = "" AND nv_si = 0 THEN DO:
                    FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                            maktab_fil.sclass  = nv_class        AND 
                            maktab_fil.makdes  = nv_makdes       AND 
                            maktab_fil.moddes  = rm_moddes       AND 
                            maktab_fil.makyea  = nv_caryear      AND
                            maktab_fil.tons    <= 4              NO-LOCK  :
                        rm_modcod = maktab_fil.modcod.
                        IF rm_modcod <> "" THEN LEAVE loop_modcod.
                    END.
                END.
            END.
            /* 4.1 - 12 ton*/
            ELSE IF nv_ton > 4 AND nv_ton <= 12  THEN DO:
                FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                    maktab_fil.sclass  = nv_class     AND 
                    maktab_fil.makdes  = nv_makdes    AND 
                    (INDEX(maktab_fil.moddes,rm_moddes) <> 0   OR
                     INDEX(rm_moddes,maktab_fil.moddes) <> 0 ) AND /*A64-0369*/
                    maktab_fil.makyea  = nv_caryear    AND
                    maktab_fil.tons    > 4             AND 
                    maktab_fil.tons    <= 12           NO-LOCK BY maktab_fil.si :
                    IF nv_si < maktab_fil.si AND nv_redbookmin = "" THEN ASSIGN nv_redbookmin = maktab_fil.modcod nv_simin = maktab_fil.si .
                   nv_rsimax = maktab_fil.si + ((maktab_fil.si * nv_simaxp) / 100).
                   nv_rsimin = maktab_fil.si - ((maktab_fil.si * nv_siminp) / 100).
                ...
                IF rm_modcod = "" AND nv_si = 0 THEN DO:
                    FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                            ...
                            maktab_fil.makyea  = nv_caryear      AND
                            maktab_fil.tons    > 4               AND
                            maktab_fil.tons    <= 12            NO-LOCK  :
                        rm_modcod = maktab_fil.modcod.
                        IF rm_modcod <> "" THEN LEAVE loop_modcod.
                    END.
                END.
            END. /* end >4 <= 12 */
            ELSE DO: /* มากกว่า 12 ตัน */
                FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                    maktab_fil.sclass  = nv_class     AND 
                    maktab_fil.makdes  = nv_makdes    AND 
                    (INDEX(maktab_fil.moddes,rm_moddes) <> 0   OR
        ...
        ELSE IF nv_engcc <= 2000 THEN DO:
            FOR EACH stat.maktab_fil USE-INDEX maktab02 WHERE
                    ...
                    (INDEX(maktab_fil.moddes,rm_moddes) <> 0   OR
                     INDEX(rm_moddes,maktab_fil.moddes) <> 0 ) AND /*A64-0369*/
                    maktab_fil.makyea  = nv_caryear AND
                    maktab_fil.engine <= 2000     NO-LOCK BY maktab_fil.si :

...
&Scoped-Define ENABLED-OBJECTS fi_camname fi_pack ra_typefile fi_loaddat ~
fi_branch fi_producer fi_agent fi_prevbat fi_bchyr fi_filename bu_file ~
fi_output1 fi_output2 fi_output3 fi_usrcnt fi_usrprem buok bu_exit bu_hpbrn ~
bu_hpacno72 bu_hpagent72 fi_process RECT-370 RECT-372 RECT-373 RECT-374 ~
...
DEFINE FRAME fr_main
     ...
     "Success Net Premium :" VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 2 FONT 6
     "                                                      IMPORT TEXT FILE TIL[TPIS]" VIEW-AS TEXT
...
/* SETTINGS FOR FILL-IN fi_usrprem IN FRAME fr_main
   ALIGN-L                                                              */
/* SETTINGS FOR TEXT-LITERAL "                                                      IMPORT TEXT FILE TIL[TPIS]"
          SIZE 128 BY 1 AT ROW 1.29 COL 129.67 RIGHT-ALIGNED            */
...
ON END-ERROR OF c-Win /* Load Text  Generate TPIS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-Win c-Win
ON WINDOW-CLOSE OF c-Win /* Load Text  Generate TPIS */
DO:
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "wgwttpis".
  gv_prog  = "Load Text & Generate TPIS".
  fi_loaddat = TODAY.
  DISP fi_loaddat WITH FRAME fr_main.
  ...
  ASSIGN
      ...
      fi_process    = "Load Text file TPIS [renew]"    
      ra_typefile   = 1   
      fi_bchyr    = YEAR(TODAY) .
...
------------------------------------------------------------------------------*/
  ...
  ENABLE fi_camname fi_pack ra_typefile fi_loaddat fi_branch fi_producer 
         ...
         bu_hpagent72 fi_process RECT-370 RECT-372 RECT-373 RECT-374 RECT-376 
...
DO: 
    ...
    REPEAT:     
        IMPORT DELIMITER "|" 
            nv_reportDate       /*REPORT_DATE*/         
            nv_dateReq          /*INS_REQUEST*/         
            nv_contract         /*CONTRACT*/            
            nv_refno            /*REF_NO*/              
            nv_title            /*TITLE*/               
            nv_custName         /*CUSTOMER_NAME*/       
            nv_address1         /*MAILING_ADDRESS1*/    
            nv_address2         /*MAILING ADDRESS2*/    
            nv_address3         /*MAILING_ADDRESS3*/    
            nv_postCode         /*MAILING_POSTAL_CODE*/ 
            nREG_ADDRESS1       /*REG_ADDRESS1*/        
            nREG_ADDRESS2       /*REG_ADDRESS2*/        
            nREG_ADDRESS3       /*REG_ADDRESS3*/        
            nREG_CODE           /*REG_POSTAL_CODE*/     
            nv_icno             /*NATIONALITY*/         
            nv_birthDate        /*BIRTH*/               
            nv_tel              /*H_TEL*/               
            nv_brand            /*BRAND*/               
            ...
            nv_DATE724          /*DATE724*/                       
            nYR_EXT             /*YR_EXT*/                        
            nINS_COLL_OLD_CONT  /*INS_COLL_OLD_CONT*/              
            nNEW_OLD            /*NEW_OLD*/                      
            nINS_RQST           /*INS_RQST*/                      
            nENDORSE            /*ENDORSE*/                       
            nYR_EXT2            /*YR_EXT*/                         
            nINS_COLL           /*INS_COLL*/  /*nv_LPBName*/
            nv_ISP
            nv_f18_1
            nv_f18_2
            nv_f18_3
        ...
        IF index(nv_reportDate,"REPORT") <> 0 THEN RUN proc_assigninit.
        ELSE IF  nv_reportDate = ""           THEN RUN proc_assigninit.
        ELSE DO:
            IF nv_contract <> ""  THEN DO:    
               RUN proc_province_bran.
               FIND FIRST wdetail WHERE wdetail.policy = "0TS" + trim(nv_contract)   NO-ERROR NO-WAIT.   
                IF NOT AVAIL wdetail THEN DO:
                    CREATE wdetail.
                    ASSIGN
                        wdetail.policy      = "0TS" + trim(nv_contract) 
                        wdetail.cedpol      = trim(nv_contract) 
                        wdetail.insrefno    = ""
                        wdetail.poltyp      = "V70" 
                        wdetail.camName     = fi_camname + n_garage
                        wdetail.tiname      = trim(nv_title) 
                        wdetail.insnam      = TRIM(SUBSTRING(nv_custName,1,INDEX(nv_custName," "))) + " " + trim(SUBSTRING(nv_custName,INDEX(nv_custName," ")))
                        wdetail.iadd1       = trim(nv_address1)  
                        wdetail.iadd2       = trim(nv_address2)
                        wdetail.iadd3       = trim(nv_address3)+ " " + trim(nv_postCode)
                        wdetail.Birth       = trim(nv_birthDate)  
                        wdetail.Tele        = trim(SUBSTRING(nv_tel,1,INDEX(nv_tel," "))) + " " + trim(SUBSTRING(nv_tel,1,INDEX(nv_tel," ")))        
                        wdetail.ICNO        = trim(SUBSTRING(nv_icno,1,13)) 
                        wdetail.brand       = trim(nv_brand)  
                        wdetail.model       = TRIM(nv_modelSaf) 
                        wdetail.caryear     = trim(nv_modelYear) 
                        wdetail.cc          = STRING(np_cc)
                        wdetail.vehuse      = "1"                          /* ประเภทการใช้งานรถ */
                        ...
                        wdetail.comdat      = trim(nv_comDate)    
                        wdetail.expdat      = trim(nv_expDate)  
                        wdetail.covcod      = IF TRIM(nv_cover) = "" THEN "1" ELSE TRIM(nv_cover)  /* ประเภทประกันภัย */
                        wdetail.prempa      = fi_pack   
                        wdetail.subclass    = IF TRIM(nv_typeVeh) = "1" THEN "110"
                                              ELSE IF TRIM(nv_typeVeh) = "2" THEN "210"
                                              ELSE IF TRIM(nv_typeVeh) = "3" THEN "320"
                                              ELSE ""
                        wdetail.seat        = IF wdetail.subclass = "320" THEN "3" 
                                              ELSE IF wdetail.subclass = "110" THEN "7"
                                              ELSE "12"
                        wdetail.garage      = n_garage      /* ซ๋อมอู๋ = "" ซ่อมห้าง = G */
                        wdetail.inspec      = IF trim(nv_ISP) <> "" THEN "Y"
                                              ELSE ""
                        wdetail.stk         = ""
                        wdetail.branch      = TRIM(nv_branSaf) /*-สาขา-*/
                        wdetail.comment     = ""
                        ...
                        FIND FIRST brstat.msgcode WHERE 
                            brstat.msgcode.compno = "999"  AND brstat.msgcode.MsgDesc  = wdetail.tiname   NO-LOCK NO-WAIT NO-ERROR.
                        IF AVAIL brstat.msgcode THEN  ASSIGN wdetail.tiname  =  trim(brstat.msgcode.branch).
                        ELSE wdetail.tiname = "คุณ".
                        
                        
                        IF  nv_ISP   <> "" OR nv_f18_1 <> "" OR
                            nv_f18_2 <> "" OR nv_f18_3 <> "" OR
                            nv_f18_4 <> "" OR nv_f18_5 <> "" THEN DO:
                        FIND FIRST   wdetailmemo WHERE wdetailmemo.policy  = "0TS" + trim(nv_contract) NO-ERROR NO-WAIT.
                        IF NOT AVAIL wdetailmemo THEN DO:
                            CREATE wdetailmemo.
                            ASSIGN 
                             wdetailmemo.policy       = "0TS" + trim(nv_contract) 
...
FIND sic_bran.uwm100   Where Recid(sic_bran.uwm100)  = s_recid1
  ...
  IF sic_bran.uwm120.com1ae   = Yes  Then do:                   /* MOTOR COMMISION */
     If sic_bran.uwm120.com1p <> 0  Then nv_com1p  = sic_bran.uwm120.com1p.
           nv_com1p  =  nv_com1p - DECI(nv_dsspcvar2).
           IF nv_com1p < 0 THEN ASSIGN nv_com1p = 0.     
...
REPEAT:   
    ...
    IMPORT DELIMITER "|" 
        ...
        wdetail3.H_TEL                 /*  H_TEL */            
        wdetail3.BRAND                 /*  BRAND */            
        wdetail3.MODELTPIS             /*  MODELTPIS */            
        wdetail3.MODELSAF              /*  MODELSafety */     
        wdetail3.nYEAR                 /*  YEAR */        
        wdetail3.ncolor                /*  A57-0433 */
        wdetail3.CC                    /*  CC  */                    
        wdetail3.ENG_NUM               /*  ENG_NUM */ 
        ...
        wdetail3.LICENCE               /*  LICENCE */         
        wdetail3.CHANGWAT              /*  CHANGWAT */
        wdetail3.nCoverage             /* Coverage */      
        wdetail3.TYPE_VEH              /*  TYPE_VEH */     
        wdetail3.INS_COM               /*  INS_COM */   
        wdetail3.DATE_INS              /*  DATE_INS */      
        wdetail3.INS_EXP               /*  INS_EXP */         
        wdetail3.POL_NUM               /*  POL_NUM */    
        wdetail3.stk_no                /*  A57-0433*/
        wdetail3.INS_AMT               /*  INS_AMT */ 
        wdetail3.PREMIUM               /*  PREMIUM */ 
        wdetail3.NET_PREM              /*  NET_PREM */ 
        wdetail3.CAT3                  /*  CAT3  */ 
        ...
        wdetail3.nINS_EXP1             /*  A57-0433 */   
        wdetail3.nPOL_NUM1             /*  A57-0433 */   
        wdetail3.nPREMIUM1             /*  A57-0433 */ /*wdetail3.LPBNAME   /*  LPBNAME */ */         
        wdetail3.MKT_OFF               /*  MKT_OFF */          
        wdetail3.OLD_CONT              /*  OLD_CONT */      
        wdetail3.DATE491               /*  DATE491 */                                    
        wdetail3.DATE724               /*  DATE724, */ 
...
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X4;K"  '"'  "CONTRACT"          '"' SKIP.                                              
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X5;K"  '"'  "REF_NO "           '"' SKIP.                            
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X6;K"  '"'  "TITLE  "           '"' SKIP.                                                      
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X7;K"  '"'  "CUSTOMER_NAME  "   '"' SKIP.                            
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X8;K"  '"'  "MAILING_ADDRESS1"  '"' SKIP.                                         
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X9;K"  '"'  "MAILING_ADDRESS2"  '"' SKIP.                     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X10;K" '"'  "MAILING_ADDRESS3"  '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X11;K" '"'  "POSTAL_CODE"       '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X12;K" '"'  "REG_ADDRESS1"      '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X13;K" '"'  "REG_ADDRESS2"      '"' SKIP.  
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X14;K" '"'  "REG_ADDRESS3"      '"' SKIP.                     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X15;K" '"'  "REG_POSTAL_CODE"   '"' SKIP.                     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X16;K" '"'  "NATIONALITY_ID "   '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X17;K" '"'  "BIRTH "            '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X18;K" '"'  "Phone "            '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X19;K" '"'  "BRAND "            '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X20;K" '"'  "MODELTPIS"         '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X21;K" '"'  "MODELSAF "         '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X22;K" '"'  "YEAR "             '"' SKIP.     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X23;K" '"'  "COLOUR"            '"' SKIP.     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X24;K" '"'  "CC "               '"' SKIP.                     
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X25;K" '"'  "ENG_NUM  "         '"' SKIP.                      
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X26;K" '"'  "CHASSIS  "         '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X27;K" '"'  "LICENCE  "         '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X28;K" '"'  "CHANGWAT "         '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X29;K" '"'  "Coverage "         '"' SKIP.                                           
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X30;K" '"'  "TYPE_VEH "         '"' SKIP.                               
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X31;K" '"'  "INS_COM  "         '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X32;K" '"'  "DATE_INS "         '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X33;K" '"'  "INS_EXP  "         '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X34;K" '"'  "POL_NUM  "         '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X35;K" '"'  "Sticker No. "      '"' SKIP.                                                
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X36;K" '"'  "INS_AMT  "         '"' SKIP.                                      
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X37;K" '"'  "PREMIUM  "         '"' SKIP.                                        
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X38;K" '"'  "NET_PREM "         '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X39;K" '"'  "CAT3   "           '"' SKIP.                    
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X40;K" '"'  "INS_COM1"          '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X41;K" '"'  "INSRQST1"          '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X42;K" '"'  "INS_EXP1"          '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X43;K" '"'  "POL_NUM1"          '"' SKIP. 
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X44;K" '"'  "PREMIUM1"          '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X45;K" '"'  "MKT_OFF"           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X46;K" '"'  "OLD_CONT"          '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X47;K" '"'  "DATE491"           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X48;K" '"'  "DATE724"           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X49;K" '"'  "YR_EXT"            '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X50;K" '"'  "INS_COLL_OLD_CONT" '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X51;K" '"'  "NEW_OLD"           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X52;K" '"'  "INS_RQST"          '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X53;K" '"'  "ENDORSE"           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X54;K" '"'  "YR_EXT"            '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X55;K" '"'  "INS_COLL"          '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X56;K" '"'  "กล่องตรวจสภาพ "    '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X57;K" '"'  "F18_1  "           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X58;K" '"'  "F18_2  "           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X59;K" '"'  "F18_3  "           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X60;K" '"'  "F18_4  "           '"' SKIP.
PUT STREAM ns2 "C;Y" STRING(nv_row) ";X61;K" '"'  "F18_5  "           '"' SKIP.

FOR EACH wdetail3 WHERE wdetail3.CONTRACT <> "" .
    IF TRIM(wdetail3.CHANGWAT) <> "" THEN RUN proc_province_2.
    FIND LAST sicuw.uwm100 USE-INDEX uwm10002    WHERE 
        sicuw.uwm100.cedpol = wdetail3.CONTRACT  AND 
        sicuw.uwm100.poltyp = "V70"              NO-LOCK NO-ERROR .
    IF AVAIL sicuw.uwm100 THEN
        ASSIGN wdetail3.policy = sicuw.uwm100.policy.
    ELSE wdetail3.policy = "".
    ASSIGN nv_row          =  nv_row + 1 
        wdetail3.MODELSAF  = IF index(trim(wdetail3.MODELTPIS),"TFR86")  <> 0 THEN "D-MAX" ELSE "".
                                                                                            
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X1;K"  '"'  wdetail3.REPORT_DATE        '"' SKIP.   /*  REPORT_DATE */         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X2;K"  '"'  wdetail3.INS_REQUEST        '"' SKIP.   /*  INS_REQUEST */         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X3;K"  '"'  wdetail3.policy             '"' SKIP.   /*  Policy   */
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X4;K"  '"'  wdetail3.CONTRACT           '"' SKIP.   /*  CONTRACT    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X5;K"  '"'  wdetail3.REF_NO             '"' SKIP.   /*  REF_NO  */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X6;K"  '"'  wdetail3.nTITLE             '"' SKIP.   /*  TITLE   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X7;K"  '"'  wdetail3.CUSTOMER_NAME      '"' SKIP.   /*  CUSTOMER_NAME   */                 
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X8;K"  '"'  wdetail3.MAILING_ADDRESS1   '"' SKIP.   /*  MAILING_ADDRESS1    */             
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X9;K"  '"'  wdetail3.MAILING_ADDRESS2   '"' SKIP.   /*  MAILING_ADDRESS2    */             
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X10;K" '"'  wdetail3.MAILING_ADDRESS3   '"' SKIP.   /*  MAILING_ADDRESS3    */             
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X11;K" '"'  wdetail3.POSTAL_CODE        '"' SKIP.   /*  POSTAL_CODE */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X12;K" '"'  wdetail3.REG_ADDRESS1       '"' SKIP.   /*  NATIONALITY_ID  */                 
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X13;K" '"'  wdetail3.REG_ADDRESS2       '"' SKIP.   /*  BIRTH   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X14;K" '"'  wdetail3.REG_ADDRESS3       '"' SKIP.   /*  H_TEL   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X15;K" '"'  wdetail3.REG_CODE           '"' SKIP.   /*  BRAND   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X16;K" '"'  wdetail3.NATIONALITY_ID     '"' SKIP.   /*  MODELTPIS   */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X17;K" '"'  wdetail3.BIRTH              '"' SKIP.   /*  MODELSAF    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X18;K" '"'  wdetail3.H_TEL              '"' SKIP.   /*  YEAR    */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X19;K" '"'  wdetail3.BRAND              '"' SKIP.   /*  CC  */                             
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X20;K" '"'  wdetail3.MODELTPIS          '"' SKIP.   /*  Coverage    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X21;K" '"'  wdetail3.MODELSAF           '"' SKIP.   /*  TYPE_VEH    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X22;K" '"'  wdetail3.nYEAR              '"' SKIP.   /*  INS_COM */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X23;K" '"'  wdetail3.ncolor             '"' SKIP.   /*  LICENCE */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X24;K" '"'  wdetail3.CC                 '"' SKIP.   /*  CHANGWAT    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X25;K" '"'  wdetail3.ENG_NUM            '"' SKIP.   /*  CHASSIS */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X26;K" '"'  wdetail3.CHASSIS            '"' SKIP.   /*  ENG_NUM */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X27;K" '"'  wdetail3.LICENCE            '"' SKIP.   /*  INS_AMT */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X28;K" '"'  wdetail3.CHANGWAT           '"' SKIP.   /*  NET_PREM    */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X29;K" '"'  wdetail3.nCoverage          '"' SKIP.   /*  PREMIUM */                      
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X30;K" '"'  wdetail3.TYPE_VEH           '"' SKIP.   /*  POL_NUM */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X31;K" '"'  wdetail3.INS_COM            '"' SKIP.   /*  DATE_INS    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X32;K" '"'  wdetail3.DATE_INS           '"' SKIP.   /*  INS_EXP */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X33;K" '"'  wdetail3.INS_EXP            '"' SKIP.   /*  LPBNAME */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X34;K" '"'  wdetail3.POL_NUM            '"' SKIP.   /*  MKT_OFF */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X35;K" '"'  wdetail3.stk_no             '"' SKIP.   /*  OLD_CONT    */                     
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X36;K" '"'  wdetail3.INS_AMT            '"' SKIP.   /*  CAT3    */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X37;K" '"'  wdetail3.PREMIUM            '"' SKIP.   /*  DATE491 */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X38;K" '"'  wdetail3.NET_PREM           '"' SKIP.   /*  DATE724 */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X39;K" '"'  wdetail3.CAT3               '"' SKIP.   /*  ISP */                             
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X40;K" '"'  wdetail3.nINS_COM1          '"' SKIP.   /*  F18_1   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X41;K" '"'  wdetail3.nINSRQST1          '"' SKIP.   /*  F18_2   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X42;K" '"'  wdetail3.nINS_EXP1          '"' SKIP.   /*  F18_3   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X43;K" '"'  wdetail3.nPOL_NUM1          '"' SKIP.   /*  F18_4   */                         
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X44;K" '"'  wdetail3.nPREMIUM1          '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X45;K" '"'  wdetail3.MKT_OFF            '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X46;K" '"'  wdetail3.OLD_CONT           '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X47;K" '"'  wdetail3.DATE491            '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X48;K" '"'  wdetail3.DATE724            '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X49;K" '"'  wdetail3.nYR_EXT            '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X50;K" '"'  wdetail3.nINS_COLL_OLD_CONT '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X51;K" '"'  wdetail3.nNEW_OLD           '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X52;K" '"'  wdetail3.nINS_RQST          '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X53;K" '"'  wdetail3.nENDORSE           '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X54;K" '"'  wdetail3.nYR_EXT2           '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X55;K" '"'  wdetail3.nINS_COLL          '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X56;K" '"'  wdetail3.ISP                '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X57;K" '"'  wdetail3.F18_1              '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X58;K" '"'  wdetail3.F18_2              '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X59;K" '"'  wdetail3.F18_3              '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X60;K" '"'  wdetail3.F18_4              '"' SKIP.   /*  F18_4   */   
   PUT STREAM ns2 "C;Y" STRING(nv_row) ";X61;K" '"'  wdetail3.F18_5              '"' SKIP.   /*  F18_4   */   
END.
OUTPUT STREAM ns2 CLOSE.
FOR EACH wdetail3.
    DELETE wdetail3.

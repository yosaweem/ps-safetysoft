...
/*******************************/                                                
/*programid  : wgwtlnzi.w                                                     */ 
/*programname: Load Text NZI               */                                 
/*Copyright  : Safety Insurance Public Company Limited                        */ 
/*                         บริษัท ประกันคุ้มภัย จำกัด (มหาชน)                 */ 
/*create by  : Ranu I. date. 10/05/2016                                       
               Duplicate wgwtlbu2.w ปรับโปรแกรมให้สามารถนำเข้า Load Text NZI  */
...
DEF VAR nv_driver   AS CHAR FORMAT "x(25)" INIT "".
...
DEFINE FRAME fr_main
     ...
     fi_bchno AT ROW 22.57 COL 15 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     ...
     fi_impcnt AT ROW 22.24 COL 59.17 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     ...
     fi_premtot AT ROW 22.24 COL 97 NO-LABEL NO-TAB-STOP 
     fi_premsuc AT ROW 23.29 COL 95 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     ...
     "        Vat code  :" VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "" VIEW-AS TEXT
          SIZE 4.5 BY .95 AT ROW 6.38 COL 126.5 
          BGCOLOR 8 
     ...
     "   Total Net Premium  :" VIEW-AS TEXT
          SIZE 21.5 BY .91 AT ROW 22.24 COL 94.67 RIGHT-ALIGNED
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "Success Record :" VIEW-AS TEXT
          SIZE 17.5 BY .91 AT ROW 23.29 COL 57.67 RIGHT-ALIGNED
          BGCOLOR 8 FGCOLOR 1 FONT 6
...
DEFINE FRAME fr_main
     ...
     "     Total Record :":60 VIEW-AS TEXT
          SIZE 17.5 BY .91 AT ROW 22.24 COL 57.5 RIGHT-ALIGNED
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "   LOAD TEXT FILE MOTOR[ NZI ]" VIEW-AS TEXT
     ...
     "Total Net Premium Imp  :":60 VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "BHT.":60 VIEW-AS TEXT
          SIZE 6 BY .91 AT ROW 22.24 COL 116
          BGCOLOR 8 FGCOLOR 1 FONT 6
...
DO:
    ...
    EXPORT DELIMITER "|" 
        ...
        "Text F7 หลังจากนั้นเพิ่มตามหมายเหตุด้านล่าง"   
        "F17=P.Text02"   
        "F17=P.Text03"   
        "F18=Memo01"   
        "F18=Memo02"   
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  ASSIGN 
      gv_prgid   = "wgwtlnzi.w"
      gv_prog    = "LOAD TEXT FILE MOTOR[NZI]"
      fi_loaddat = TODAY.
  DISP fi_loaddat WITH FRAME fr_main.
  ...
  ASSIGN
      ...
      fi_agent    = ""
      fi_bchyr    = YEAR(TODAY)
      fi_process  = "LOAD TEXT FILE MOTOR[NZI]".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pd_chk_uzsusp c-Win 
PROCEDURE pd_chk_uzsusp :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF VAR nv_policy AS CHAR FORMAT "x(15)".
DEF VAR nv_rencont AS INT init 0.
DEF VAR nv_endcont AS INT init 0.
DEF VAR nv_month AS INT INIT 0.
DEF VAR n_tariff AS CHAR INIT "".
ASSIGN  fi_process = " Check Data Subspect (uzsusp)........" .
DISP fi_process  WITH FRAME fr_main.
IF ((wdetail.prepol = "" ) OR wdetail.covcod <> "T" ) THEN DO:
    FIND LAST sicuw.uzsusp USE-INDEX uzsusp01 WHERE sicuw.uzsusp.vehreg = wdetail.vehreg  NO-LOCK NO-ERROR NO-WAIT.
    IF AVAIL sicuw.uzsusp THEN DO:
        ASSIGN wdetail.supspec  = sicuw.uzsusp.suscod
               wdetail.note_sup = sicuw.uzsusp.notes
               wdetail.pass     = "N"  
               wdetail.comment  = wdetail.comment + "| Have Subspect: " + wdetail.note_sup
               wdetail.OK_GEN   = "N".
    END.
    ELSE DO:
        FIND LAST sicuw.uzsusp USE-INDEX uzsusp05 WHERE  sicuw.uzsusp.cha_no = wdetail.chassis  NO-LOCK NO-ERROR NO-WAIT.
            IF AVAIL sicuw.uzsusp THEN DO:
                ASSIGN wdetail.supspec  = sicuw.uzsusp.suscod
                       wdetail.note_sup = sicuw.uzsusp.notes
                       wdetail.pass     = "N"  
...
IF wdetail.prepol = " "  THEN DO:
    ASSIGN  n_tariff = ""
            fi_process = " Check Expire date........." .
    DISP fi_process  WITH FRAME fr_main.
    ASSIGN n_tariff = IF wdetail.poltyp = "V70" THEN "X" ELSE "9".
    FIND LAST sicuw.uwm301 USE-INDEX uwm30102 WHERE sicuw.uwm301.vehreg = wdetail.vehreg AND sicuw.uwm301.tariff = n_tariff NO-LOCK NO-ERROR NO-WAIT.
        IF AVAIL sicuw.uwm301 THEN DO:
            FIND LAST sicuw.uwm100 USE-INDEX uwm10001 WHERE sicuw.uwm100.policy = sicuw.uwm301.policy AND
                                         ...
                                         sicuw.uwm100.endcnt = sicuw.uwm301.endcnt NO-LOCK NO-ERROR NO-WAIT.
                 IF AVAIL sicuw.uwm100 THEN DO:
                     IF sicuw.uwm100.expdat <=  TODAY  THEN DO:
                        IF YEAR(sicuw.uwm100.expdat) = YEAR(TODAY) THEN DO:
                            nv_month = MONTH(TODAY) - MONTH(sicuw.uwm100.expdat).
                            IF nv_month <=  3 /*AND sicuw.uwm100.rencnt > 0*/ THEN 
                                ASSIGN  wdetail.pass     = "N"  
                                        wdetail.comment  = wdetail.comment + "| กรมธรรม์หมดอายุยังไม่เกิน 3 เดือน "
                                        wdetail.OK_GEN   = "N".
                            ELSE
                        ...
                        END. /*year = today */
                        ELSE DO:
                           IF YEAR(sicuw.uwm100.expdat) = YEAR(TODAY) - 1 THEN DO:
                                IF MONTH(TODAY) = 1 AND MONTH(sicuw.uwm100.expdat) > 9 /* AND sicuw.uwm100.rencnt > 0*/ THEN
                                    ASSIGN  wdetail.pass     = "N"  
                                            wdetail.comment  = wdetail.comment + "| กรมธรรม์หมดอายุยังไม่เกิน 3 เดือน "
                                            wdetail.OK_GEN   = "N".
                                ELSE IF MONTH(TODAY) = 2 AND MONTH(sicuw.uwm100.expdat) > 10 /*AND sicuw.uwm100.rencnt > 0*/ THEN
                                    ASSIGN  wdetail.pass     = "N"  
                                            wdetail.comment  = wdetail.comment + "| กรมธรรม์หมดอายุยังไม่เกิน 3 เดือน "
                                            wdetail.OK_GEN   = "N".
                                ELSE IF MONTH(TODAY) = 3 AND MONTH(sicuw.uwm100.expdat) > 11 /*AND sicuw.uwm100.rencnt > 0*/ THEN
                                    ASSIGN  wdetail.pass     = "N"  
                                            wdetail.comment  = wdetail.comment + "| กรมธรรม์หมดอายุยังไม่เกิน 3 เดือน "
                                            wdetail.OK_GEN   = "N".
                                ELSE 
                    ...
                    ELSE DO:
                        ASSIGN  wdetail.pass     = "N"  
                                wdetail.comment  = wdetail.comment + "|" + sicuw.uwm100.policy + " กรุณาตรวจสอบข้อมูลความคุ้มครองของกรมธรรม์ในระบบพรีเมียมอีกครั้ง"
                                wdetail.OK_GEN   = "N".
                    END.
                END. /* avail uwm100*/
                ELSE DO:
        ...
        ELSE DO:
            /*ASSIGN  wdetail.pass     = "Y"
                    wdetail.OK_GEN   = "Y" . */
            FIND LAST sicuw.uwm301 USE-INDEX uwm30121 WHERE sicuw.uwm301.cha_no = wdetail.chassis AND sicuw.uwm301.tariff = n_tariff NO-LOCK NO-ERROR NO-WAIT.
                IF AVAIL sicuw.uwm301 THEN DO:
                    FIND LAST sicuw.uwm100 USE-INDEX uwm10001 WHERE sicuw.uwm100.policy = sicuw.uwm301.policy AND
                                                 sicuw.uwm100.rencnt = sicuw.uwm301.rencnt AND
                                                 sicuw.uwm100.endcnt = sicuw.uwm301.endcnt NO-LOCK NO-ERROR NO-WAIT.
...
ASSIGN wdetail.PASS = "Y"
    wdetail.tariff  = "9"
    wdetail.compul  = "y" 
    fi_process      = "Process data load text file....compulsary "   
    nv_siredbook    = 0
    nv_class72rd    = ""
...
IF wdetail.vehreg = " " THEN
    wdetail.vehreg = IF SUBSTR(wdetail.chassis,LENGTH(wdetail.chassis) - 8) <> "" THEN
                     ...
                     ELSE "".
IF DATE(wdetail.comdat) < TODAY  THEN 
 ASSIGN 
 wdetail.comment       = wdetail.comment + "| วันที่คุ้มครองต้องไม่น้อยกว่าวันที่ปัจจุบัน"
 wdetail.pass          = "N"   
 wdetail.OK_GEN        = "N". 
...
DO:
    ...
    REPEAT:
        IMPORT DELIMITER "|" 
            ...
            n_textf5            /*Text F17 หลังจากนั้นเพิ่มตามหมายเหตุด้านล่าง     */  
            n_textf52           /*F17=P.Text02                                     */  
            n_textf53           /*F17=P.Text03                                     */  
            n_textf7            /*F18=Memo01                                       */  
            n_textf72           /*F18=Memo02                                       */  
            n_textf73           /*F18=Memo03                                       */  
            n_com70             /*com 70                                           */  
            ...
            n_cpol              /*เลขที่อ้างอิง                                    */  
            n_camp              /*แคมเปญ                                           */  
            n_inspec            /*ตรวจสภาพ                                         */  
            n_driv              /*จำนวนผู้ขับขี่                                   */  
            n_drivnam1          /*ผู้ขับขี่ 1                                      */  
            n_drivnam2          /*ผู้ขับขี่ 2                                      */  
            n_dbdate1           /*วันเกิดผู้ขับขี่1                                */  
            n_dbdate2           /*วันเกิดผู้ขับขี่2                                */  
            n_occup             /*อาชีพผู้เอาประกัน */
            n_occdriv1          /*อาชีพผู้ขับขี่1 */
            n_occdriv2 .         /*อาชีพผู้ขับขี่2 */
            /*n_textacc     
            n_textacc1    
            n_textacc2    
            n_textacc3    
...
IF  CONNECTED("sic_exp") THEN DO:
    /*---- create by : A59-0060-----*/
    RUN wgw\wgwnziex(INPUT-OUTPUT  wdetail.prepol,      
                     INPUT-OUTPUT  wdetail.n_branch,               
                     INPUT-OUTPUT  re_insref,           
                     INPUT-OUTPUT  re_title ,            
                     INPUT-OUTPUT  re_name1 ,                
                     INPUT-OUTPUT  wdetail.name2,            
                     INPUT-OUTPUT  wdetail.name3,          
                     INPUT-OUTPUT  re_addr1,       
                     INPUT-OUTPUT  re_addr2,          
                     INPUT-OUTPUT  re_addr3,          
                     INPUT-OUTPUT  re_addr4,
                     INPUT-OUTPUT  re_occup,
...
------------------------------------------------------------------------------*/
/*---- create by : A59-0060-----*/
    ASSIGN  wdetail.firstdat = re_firstdat.
    IF  wdetail.comdat   = ""   THEN  ASSIGN wdetail.comdat   =  re_comdat .     
    IF  wdetail.expdat   = ""   THEN  ASSIGN wdetail.expdat   =  re_expdat . 
    if  wdetail.inserf   = ""   THEN  ASSIGN wdetail.inserf   =  re_insref . 
    if  wdetail.tiname   = ""   THEN  ASSIGN wdetail.tiname   =  re_title .     
    if  wdetail.insnam   = ""   THEN  ASSIGN wdetail.insnam   =  re_name1 .    
    if  wdetail.n_addr1  = ""   THEN  ASSIGN wdetail.n_addr1  =  re_addr1  .     
    if  wdetail.n_addr2  = ""   THEN  ASSIGN wdetail.n_addr2  =  re_addr2 .      
    if  wdetail.n_addr3  = ""   THEN  ASSIGN wdetail.n_addr3  =  re_addr3  .    
    if  wdetail.n_addr4  = ""   THEN  ASSIGN wdetail.n_addr4  =  re_addr4 .
    IF  wdetail.occup    = ""   THEN  ASSIGN wdetail.occup    =  re_occup.
    ...
    IF  nv_driver = "" THEN ASSIGN nv_driver = re_driver.
...
IF wdetail.poltyp = "v70" THEN DO:
   ASSIGN fi_process = "Create data to base..." + wdetail.policy 
    ...
    ASSIGN chk = NO
        NO_basemsg = " " .
    If wdetail.drivnam = "N" OR nv_drivno = 0  Then 
        Assign nv_drivvar   = " "
        nv_drivcod   = "A000"
...
END.
IF wdetail.n_branch = "" THEN 
    ASSIGN wdetail.comment = wdetail.comment + "| สาขาเป็นค่าว่างกรุณาใส่สาขาอผู้เอาประกัน"
    wdetail.pass    = "N"     
    wdetail.OK_GEN  = "N".
IF index("123456789",substr(wdetail.policy,1,1)) <> 0  THEN DO:
    IF wdetail.n_branch <> (substr(wdetail.policy,1,2)) THEN 
...
IF wdetail.drivnam  = "y" AND wdetail.drivnam1  =  " "   THEN 
    ...
    wdetail.pass    = "N" 
    WDETAIL.OK_GEN  = "N".
IF wdetail.supspec <> " " OR wdetail.note_sup <> " "  THEN
    ASSIGN wdetail.comment  = wdetail.comment + "| Have Subspect" + wdetail.note_sup
           wdetail.pass     = "N"  
           wdetail.OK_GEN   = "N".
...
IF NOT AVAILABLE uwm130 THEN 
    ...
    ASSIGN sic_bran.uwm301.policy = sic_bran.uwm120.policy                 
      ...
      /*sic_bran.uwm301.trareg    = trim(wdetail.vehreg)*/  /*A58-0206*/
        ...
        sic_bran.uwm301.logbok    = TRIM(wdetail.inspec)
  ...
  IF trim(wdetail.drivnam) = "Y"  THEN DO:      /* drive name new */
      RUN proc_mailtxt.                             
  END.                                              
  ELSE DO:
    IF nv_driver = ""  THEN ASSIGN nv_drivno = 0. /* driver name renew */
    ELSE DO:
...
IF NOT AVAIL wdetail THEN DO:
    ...
    ASSIGN 
        ...
        wdetail.cpol       = TRIM(n_cpol)
        wdetail.camp       = TRIM(n_camp)
        wdetail.inspec     = TRIM(n_inspec)
        wdetail.drivnam    = TRIM(n_driv)
        wdetail.drivnam1   = trim(n_drivnam1) 
        wdetail.drivnam2   = trim(n_drivnam2) 
        wdetail.dbdate1    = trim(n_dbdate1) 
        wdetail.dbdate2    = trim(n_dbdate2)
        wdetail.occup      = TRIM(n_occup)
        wdetail.occdriv1   = trim(n_occdriv1)    
        wdetail.occdriv2   = trim(n_occdriv2).  
    IF TRIM(np_addr1) <> "" THEN DO:
       DO WHILE INDEX(np_addr1,"  ") <> 0 :
            ASSIGN np_addr1 = REPLACE(np_addr1,"  "," ").
        END.
        IF LENGTH(np_addr1) > 35  THEN DO:
            loop_add01:
            DO WHILE LENGTH(np_addr1) > 35 :
                IF r-INDEX(np_addr1," ") <> 0 THEN DO:
                    ASSIGN 
                        np_addr2  = trim(SUBSTR(np_addr1,r-INDEX(np_addr1," "))) + " " + TRIM(np_addr2)
                        np_addr1  = trim(SUBSTR(np_addr1,1,r-INDEX(np_addr1," "))).
                END.
                ELSE LEAVE loop_add01.
            END.
            loop_add02:
            DO WHILE LENGTH(np_addr2) > 35 :
                IF r-INDEX(np_addr2," ") <> 0 THEN DO:
                    ASSIGN 
                        np_addr3          = trim(SUBSTR(np_addr2,r-INDEX(np_addr2," "))) + " " + TRIM(np_addr3)
                        np_addr2   = trim(SUBSTR(np_addr2,1,r-INDEX(np_addr2," "))) .
                   END.
                ELSE LEAVE loop_add02.
            END.
            loop_add03:
            DO WHILE LENGTH(np_addr3) > 35 :
                IF r-INDEX(np_addr3," ") <> 0 THEN DO:
                    ASSIGN 
                        np_addr4   = trim(SUBSTR(np_addr3,r-INDEX(np_addr3," "))) + " " + TRIM(np_addr4)
                        np_addr3   = trim(SUBSTR(np_addr3,1,r-INDEX(np_addr3," "))) .
                END.
                ELSE LEAVE loop_add03.
            END.
        END.
    END.
    ASSIGN wdetail.n_addr1    = trim(np_addr1)
...
FIND FIRST wdetail WHERE wdetail.policy = trim(n_policy72) NO-ERROR NO-WAIT.
    IF NOT AVAIL wdetail THEN DO:
        ...
        ASSIGN 
            ...
            wdetail.engno       = trim(n_engno)             
            wdetail.caryear     = trim(n_caryear)
            wdetail.firstdat    = IF trim(n_comdat72) = "" THEN trim(n_comdat1) else trim(n_comdat72)   
            wdetail.comdat      = IF trim(n_comdat72) = "" then trim(n_comdat1) else trim(n_comdat72)
            wdetail.expdat      = IF trim(n_expdat72) = "" then trim(n_expdat1) else trim(n_expdat72)
            wdetail.benname     = trim(n_benname) 
            wdetail.prmtxt      = ""
            wdetail.pass        = "Y"
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF VAR nv_sex1 AS CHAR FORMAT "x(10)".
DEF VAR nv_sex2 AS CHAR FORMAT "x(10)".

IF  wdetail.drivnam = "Y" AND wdetail.drivnam1 <> " " THEN DO :      /*note add 07/11/2005*/
    ...
    DEF VAR no_itemno AS CHAR FORMAT "999".
     ...
     IF wdetail.dbdate1 <> " "  AND wdetail.drivnam1 <> " " THEN DO: /*note add & modified*/
        nv_drivage1 =  (YEAR(TODAY) + 543) - INT(SUBSTR(wdetail.dbdate1,7,4)).
        nv_drivbir1      = STRING(SUBSTR(wdetail.dbdate1,7,4)).
        wdetail.dbdate1 = SUBSTR(wdetail.dbdate1,1,6) + nv_drivbir1.
    
        IF INDEX(wdetail.drivnam1,"นาย") <> 0 OR INDEX(wdetail.drivnam1,"Mrs") <> 0 THEN ASSIGN nv_sex1  = "MALE".
        ELSE IF INDEX(wdetail.drivnam1,"นาง") <> 0 OR INDEX(wdetail.drivnam1,"Miss") <> 0 THEN ASSIGN nv_sex1  = "FEMALE".
        ELSE ASSIGN nv_sex1  = "FEMALE".
     END.
    
     IF wdetail.dbdate2 <> " " AND wdetail.drivnam2 <> " " THEN DO: /*note add & modified */
        nv_drivage2 =  (YEAR(TODAY) + 543) - INT(SUBSTR(wdetail.dbdate2,7,4)).
        nv_drivbir2      = STRING(SUBSTR(wdetail.dbdate2,7,4)).
        wdetail.dbdate2 = SUBSTR(wdetail.dbdate2,1,6) + nv_drivbir2.
    
        IF INDEX(wdetail.drivnam2,"นาย") <> 0 OR INDEX(wdetail.drivnam2,"Mrs") <> 0 THEN ASSIGN nv_sex1  = "MALE".
        ELSE IF INDEX(wdetail.drivnam2,"นาง") <> 0 OR INDEX(wdetail.drivnam2,"Miss") <> 0 THEN ASSIGN nv_sex1  = "FEMALE".
        ELSE ASSIGN nv_sex1  = "FEMALE".
     END.
           ...
           IF NOT AVAIL brstat.mailtxt_fil THEN DO:  
               ...
               IF NOT AVAIL brstat.mailtxt_fil   THEN DO:
                 ...
                 ASSIGN                                           
                        ...
                        SUBSTR(brstat.mailtxt_fil.ltext,51,6)     = nv_sex1
                        brstat.mailtxt_fil.ltext2    = wdetail.dbdate1 + "  " + string(nv_drivage1)
                        nv_drivno                    = 1
                        brstat.mailtxt_fil.bchyr = nv_batchyr 
                        brstat.mailtxt_fil.bchno = nv_batchno 
                        brstat.mailtxt_fil.bchcnt  = nv_batcnt 
                        SUBSTRING(brstat.mailtxt_fil.ltext2,16,30) = IF TRIM(wdetail.occdriv1) <> " " THEN TRIM(wdetail.occdriv1) ELSE "-".
                   IF wdetail.drivnam2 <> " "  THEN DO:
                       ...
                       ASSIGN
                           ...
                           SUBSTR(brstat.mailtxt_fil.ltext,51,6)     = nv_sex2
                           brstat.mailtxt_fil.ltext2   = wdetail.dbdate2 + "  " + string(nv_drivage2) 
                           nv_drivno                   = 2
                           brstat.mailtxt_fil.bchyr = nv_batchyr 
                           brstat.mailtxt_fil.bchno = nv_batchno 
                           brstat.mailtxt_fil.bchcnt  = nv_batcnt 
                           SUBSTRING(brstat.mailtxt_fil.ltext2,16,30) =  IF TRIM(wdetail.occdriv2) <> " " THEN TRIM(wdetail.occdriv2) ELSE "-"  .
                   END.  /*drivnam2 <> " " */
...
DO TRANSACTION:                                                                         
    ASSIGN                                                                             
        ...
        sic_bran.uwm100.occupn = TRIM(wdetail.occup)
        ...
        sic_bran.uwm100.opnpol = "" 
        sic_bran.uwm100.cr_1   = TRIM(wdetail.camp). 
    IF wdetail.prepol <> " " THEN DO:
        IF wdetail.poltyp = "v72"  THEN 
...
REPEAT:
  ...
  IF LASTKEY = KEYCODE("F1") OR LASTKEY = KEYCODE("ENTER") THEN DO:
      CONNECT expiry -H expiry -S expiry -ld sic_exp -N tcp -U value(gv_id) -P value(nv_pwd) NO-ERROR.

...
/*            : ปรับเพิ่มการ Match งาน Cancel                                              */
/*Modify BY   : Porntiwa P.  A58-0384  02/11/2015
              : ปรับการนำเข้างานต่ออายุตามฝั่ง Expiry                                      */
/* Modify By  : Porntiwa P.  A59-0063   15/02/2016
              : ปรับการดึงข้อมูลงานต่ออายุ                                                 */ 
/* Modify By  : Porntiwa P.   A59-0297  30/06/2016
              : ปรับเพิ่มงาน 2+ 3+                                                         */
/* Modify By  : Jiraphon P. A59-0451  26/10/2016    
              : ปรับการดึงข้อมูลต่อายุ                                                     */
/* Modify By  : Sarinya C. A61-0349  26/07/2018    
              : ปรับการแสดงข้อมูลหมายเหตุที่ออกหน้า Premium                                */
/* Modify by : Ranu I. A63-0129 แก้ไขงานที่คุ้มครองตั้งแต่ 1/04/2020 ให้ใช้ Pack T งานต่ออายุ
                หากใช้ Pack A ให้ใช้ pack เดิม อื่นๆใช้ Pack T          */
...
REPEAT:
    ...
    IMPORT DELIMITER "|" 
        np_recno           /*  1   ลำดับที่    */                              
        np_Notify_dat      /*  2   วันที่แจ้ง  */                        
        np_notifyno        /*  3   เลขรับแจ้ง  */                        
        np_branch          /*  4   Branch      */                        
        np_contract        /*  5   Contract    */                        
        np_title           /*  6   คำนำหน้าชื่อ    */                    
        np_name            /*  7   ชื่อ    */                                       
        np_name2           /*  8   นามสกุล     */                        
        np_addr1           /*  9   ที่อยู่ 1       */                    
        np_addr2           /*  10  ที่อยู่ 2       */                    
        np_addr3           /*  11  ที่อยู่ 3       */                    
        np_addr4           /*  12  ที่อยู่ 4       */                    
        np_brand           /*  13  ยี่ห้อรถ    */                        
        np_model           /*  14  รุ่นรถ      */                        
        np_vehreg          /*  15  เลขทะเบียน  */                        
        np_caryear         /*  16  ปีรถ        */                        
        np_ccweigth        /*  17  CC.         */                        
        np_cha_no          /*  18  เลขตัวถัง   */                        
        np_engno           /*  19  เลขเครื่อง  */                        
        np_codenotify      /*  20  Code ผู้แจ้ง        */                
        np_cover           /*  21  ประเภท      */                        
        np_companycode     /*  22  Code บ.ประกัน       */                
        np_prepol          /*  23  เลขกรมธรรม์เดิม     */ 
        np_idno            /*                          */
        np_comdate         /*  24  วันคุ้มครองประกัน   */                            
        np_expdate         /*  25  วันหมดประกัน        */                
        np_sumins          /*  26  ทุนประกัน   */                        
        np_premium         /*  27  ค่าเบี้ยสุทธิ์      */                
        np_premiumnet      /*  28  ค่าเบี้ยรวมภาษีอากร             */    
        np_deduct          /*  29  Deduct      */                        
        np_company72       /*  30  Code บ.ประกัน พรบ.  */                
        np_comdate72       /*  31  วันคุ้มครองพรบ.     */                                    
        np_expdate72       /*  32  วันหมดพรบ.  */                                            
        np_prmcomp         /*  33  ค่าพรบ.     */                                            
        np_drino           /*  34  ระบุผู้ขับขี่       */                                    
        np_garage          /*  35  ซ่อมห้าง    */                        
        np_access          /*  36  คุ้มครองอุปกรณ์เพิ่มเติม    */        
        np_editadd         /*  37  แก้ไขที่อยู่        */                
        np_benname         /*  38  ผู้รับผลประโยชน์    */                
        np_remak           /*  39  หมายเหตุ                            */
    ...
    IF      INDEX(np_recno, "ข้อมูลง") <> 0 THEN  NEXT.
    ELSE IF INDEX(np_recno, "mat")     <> 0 THEN  NEXT.
    ELSE IF INDEX(np_recno, "ลำดับ")   <> 0 THEN  NEXT.
...
IF  CONNECTED("sic_exp") THEN DO:
    ...
    IF wdetail.producer = "A0M0062" THEN DO:  /* กรณีงาน 12+ บวกเพิ่ม 1 เดือน */
...
IF np_title = "น.ส." THEN np_title = "นางสาว" . /*Add by Sarinya C A61-0349 09/10/2018*/
...
IF NOT AVAIL wdetail THEN DO: 
    ...
    ASSIGN 
        ...
        wdetail.compul     =  "n"
        wdetail.notifydat  =  TRIM(np_Notify_dat)      /* 2  วันที่แจ้ง   */                        
        wdetail.notifyno   =  TRIM(np_notifyno)        /* 3  เลขรับแจ้ง   */                        
        wdetail.branch     =  TRIM(np_branch)          /* 4  Branch       */                        
        wdetail.cedpol     =  TRIM(np_contract)        /* 5  Contract     */                        
        wdetail.tiname     =  TRIM(np_title)           /* 6  คำนำหน้าชื่อ */                    
        wdetail.insnam     =  TRIM(np_name)  + " " +   /* 7  ชื่อ         */                                       
                              TRIM(np_name2)           /* 8  นามสกุล      */                        
        /*wdetail.iadd1      =  TRIM(np_addr1)           /* 9  ที่อยู่ 1    */                    
        wdetail.iadd2      =  TRIM(np_addr2)           /* 10 ที่อยู่ 2    */                    
        wdetail.iadd3      =  TRIM(np_addr3)           /* 11 ที่อยู่ 3    */                    
        wdetail.iadd4      =  TRIM(np_addr4)           /* 12 ที่อยู่ 4    */ */   
        ...
        wdetail.vehreg     =  TRIM(np_vehreg)          /* 15  เลขทะเบียน  */                        
        wdetail.caryear    =  TRIM(np_caryear)         /* 16  ปีรถ        */                        
        wdetail.cc         =  TRIM(np_ccweigth)        /* 17  CC.         */                        
        wdetail.chasno     =  TRIM(np_cha_no)          /* 18  เลขตัวถัง   */                        
        wdetail.engno      =  TRIM(np_engno)           /* 19  เลขเครื่อง  */                        
        wdetail.notiuser   =  "Code ผู้แจ้ง : " + TRIM(np_codenotify)  + 
                              " Code บ.ประกัน : " + trim(np_companycode)  /*22  Code บ.ประกัน*//*  20  Code ผู้แจ้ง */                
        wdetail.covcod     =  TRIM(np_cover)           /*  21  ประเภท      */                        
        wdetail.prepol     =  TRIM(np_prepol)          /*  23  เลขกรมธรรม์เดิม     */ 
        wdetail.prempa     =  IF TRIM(np_prepol) = "" THEN 
                            (IF      (TRIM(np_cover) = "1") AND (index(TRIM(np_garage),"ซ่อมห้าง") <> 0 )  THEN "F"
                             ELSE IF (trim(np_cover) = "1") AND (index(trim(np_garage),"อู่ทั่วไป") <> 0 ) THEN "G"
        ...
        wdetail.comdat     =  TRIM(np_comdate)         /*  24  วันคุ้มครองประกัน   */                            
        wdetail.expdat     =  TRIM(np_expdate)         /*  25  วันหมดประกัน        */                
        wdetail.si         =  TRIM(np_sumins)          /*  26  ทุนประกัน   */                        
        wdetail.fi         =  TRIM(np_sumins) 
        wdetail.premt      =  TRIM(np_premium)         /*  27  ค่าเบี้ยสุทธิ์      */                
        wdetail.volprem    =  TRIM(np_premiumnet)    /*  28  ค่าเบี้ยรวมภาษีอากร */    
                           /* TRIM(np_deduct         /*  29  Deduct      */                        
                              TRIM(np_company72      /*  30  Code บ.ประกัน พรบ.  */                
                              np_comdate72           /*  31  วันคุ้มครองพรบ.     */                                    
                              np_expdate72           /*  32  วันหมดพรบ.  */                                            
                              np_prmcomp             /*  33  ค่าพรบ.     */                                             
                              np_drino    */         /*  34  ระบุผู้ขับขี่       */                                    
       wdetail.garage      =  TRIM(np_garage)        /*  35  ซ่อมห้าง    */                        
       wdetail.textf6      =  TRIM(np_access)        /*  36  คุ้มครองอุปกรณ์เพิ่มเติม    */        
       wdetail.nmember     =  TRIM(np_editadd)       /*  37  แก้ไขที่อยู่        */                
       wdetail.benname     =  TRIM(np_benname)       /*  38  ผู้รับผลประโยชน์    */ 
       wdetail.nmember2    =  TRIM(np_remak)         /*  39  หมายเหตุ                            */
       wdetail.idno        =  TRIM(np_idno)                                       
       wdetail.prekpi      =  "เบี้ยรวม KPI: " + TRIM(np_prekpi)  
       wdetail.payamount   =  "ชำระล่าสุด : " + 
...
IF NOT AVAILABLE uwm130 THEN 
    ...
    ELSE nv_newsck =  wdetail.stk.
         ...
         ASSIGN
             ...
             sic_bran.uwm301.mv_ben83  = IF INDEX(wdetail.benname,"ยกเลิก") <> 0 THEN "" ELSE TRIM(wdetail.benname) /*A61-0349*/
             sic_bran.uwm301.vehreg    = wdetail.vehreg
             sic_bran.uwm301.yrmanu    = inte(wdetail.caryear)
             sic_bran.uwm301.vehuse    = wdetail.vehuse
...
IF LENGTH(naddr1) > 29 THEN DO:
    IF (INDEX(naddr1, "ซ.") <> 0 ) THEN do: 
        n_soi = SUBSTR(naddr1,R-INDEX(naddr1,"ซ.") - 1).
        naddr1 = SUBSTR(naddr1,1,R-INDEX(naddr1,"ซ.") - 1).
    END.
    ELSE IF (INDEX(naddr1, "ซอย") <> 0 ) THEN DO:
        n_soi = SUBSTR(naddr1,R-INDEX(naddr1,"ซอย") - 1).
        naddr1 = SUBSTR(naddr1,1,R-INDEX(naddr1,"ซอย") - 1).
    END.

    IF (INDEX(naddr1, "ถ.") <> 0 ) THEN do: 
        n_road = SUBSTR(naddr1,R-INDEX(naddr1,"ถ.") - 1).
        naddr1 = SUBSTR(naddr1,1,R-INDEX(naddr1,"ถ.") - 1).
    END.
    ELSE IF (INDEX(naddr1, "ถนน") <> 0 ) THEN DO:
        n_road = SUBSTR(naddr1,R-INDEX(naddr1,"ถนน") - 1).
        naddr1 = SUBSTR(naddr1,1,R-INDEX(naddr1,"ถนน") - 1).
    END.
ASSIGN 
    naddr1 = naddr1 
    naddr2 = n_soi + n_road
...
DO:
  ...
  IF wdetail.policy <> "" THEN DO:
    ...
    IF wdetail.tiname = "น.ส." OR wdetail.tiname = "น.ส" OR wdetail.tiname = "นส." THEN wdetail.tiname = "นางสาว".
    IF wdetail.tiname = " " OR wdetail.tiname = "คุณ"  THEN 
        ASSIGN
        wdetail.comment = wdetail.comment + "| คำนำหน้าชื่อผู้เอาประกันเป็นค่าว่างหรือเป็น คุณ กรุณาแก้ไขคำนำหน้าชื่อผู้เอาประกัน"
        wdetail.pass    = "N"     
        wdetail.OK_GEN  = "N".
    ELSE DO:
        FIND FIRST brstat.msgcode WHERE 
        ...
        ELSE 
            ASSIGN
                wdetail.comment = wdetail.comment + "| คำนำหน้าชื่อผู้เอาประกันไม่ถูกต้อง กรุณาแก้ไขคำนำหน้าชื่อผู้เอาประกัน"
                wdetail.pass    = "N"     
                wdetail.OK_GEN  = "N".
    END.
    /*--------- Add By Sarinya A61-0349 ---------*/
    ...
    DO TRANSACTION:
       ...
       sic_bran.uwm100.cr_1   = TRIM(SUBSTR(wdetail.payamount,INDEX(wdetail.payamount,"ชำระล่าสุด:") + 13,12)).
       /*-- End A58-0361 --*/
...
DO TRANSACTION:
   ...
   sic_bran.uwm100.cr_1   = TRIM(SUBSTR(wdetail.payamount,INDEX(wdetail.payamount,"ชำระล่าสุด:") + 12,12)).
   ...
   IF AVAIL sic_bran.uwm301 THEN DO:
       /*IF TRIM(wdetail.benname) = "ยกเลิก8.3" THEN sic_bran.uwm301.mv_ben83 = "".*/ /*A61-0349*/
       IF INDEX(wdetail.benname,"ยกเลิก") <> 0 THEN sic_bran.uwm301.mv_ben83 = "". /*A61-0349*/
...
IF AVAIL sicuw.uwm100  THEN DO:
    ...
    ELSE DO: 
        ...
        IF AVAIL sicuw.uwm301 THEN DO:
            MESSAGE "เลขเครื่องยนต์นี้ มีการต่ออายุแล้ว" VIEW-AS ALERT-BOX .
            ASSIGN  wdetail.chasno = "Already Renew"       /*เช็คเลขเครื่องยนต์เพิ่มทำให้รู้ว่าเป็นงานต่ออายุ*/
                wdetail.comment    = wdetail.comment + "| เลขเครื่องยนต์มีการต่ออายุแล้ว" + sicuw.uwm301.cha_no
                wdetail.OK_GEN     = "N"
                wdetail.pass       = "N". 
        END.*/
...
IF LASTKEY = KEYCODE("F1") OR LASTKEY = KEYCODE("ENTER") THEN DO:
    /*CONNECT expiry -H alpha4 -S expiry -ld sic_exp -N tcp -U value(gv_id) -P value(nv_pwd) NO-ERROR. */ /*ของจริง*/  
    CONNECT expiry -H TMSTH -S expiry -ld sic_exp -N tcp -U value(gv_id) -P value(nv_pwd) NO-ERROR.  /*ของจริง*/
...
/*DO WHILE nv_line1 <= 11:*/ /*A61-0349*/
    ...
    ASSIGN 
       ...
       /* IF INDEX(wdetail.benname,"ยกเลิก") <> 0 THEN wdetail.benname = "".
        ELSE wdetail.benname = wdetail.benname. A61-0349 */
        /*IF TRIM(wdetail.benname) = "ยกเลิก8.3" OR TRIM(wdetail.benname) = "ยกเลิก 8.3" THEN wdetail.benname = "".
...
DO WHILE nv_line1 <= 14:
    ...
    IF nv_line1 = 2  THEN wuppertxt3.txt =  "ชื่อลูกค้า : " + wdetail.insnam + wdetail.insnam2 + wdetail.insnam3.   
    IF nv_line1 = 3  THEN wuppertxt3.txt =  "วันที่แจ้ง : " + wdetail.notifydat.                                          
    IF nv_line1 = 4  THEN wuppertxt3.txt =  "เลขรับแจ้ง : " + wdetail.notifyno. 
    IF nv_line1 = 5  THEN wuppertxt3.txt =  "Contract : "   + wdetail.cedpol. 
    IF nv_line1 = 6  THEN wuppertxt3.txt =  IF wdetail.nmember  = "" THEN "" ELSE "แก้ไขที่อยู่: " + wdetail.nmember. 
    IF nv_line1 = 7  THEN wuppertxt3.txt =  IF wdetail.benname  = "" THEN "" ELSE "ผู้รับผลประโยชน์: " + wdetail.benname .                                             
    /*IF nv_line1 = 8  THEN wuppertxt3.txt =  IF wdetail.nmember2 = "" THEN "" ELSE "หมายเหตุ: " + wdetail.nmember2.*/ /*A61-0349*/
    /*Add by Sarinya A61-00349*/
    IF nv_line1 = 8  THEN wuppertxt3.txt =  IF nv_text1  = "" THEN "" ELSE "หมายเหตุ: " + trim(nv_text1) .

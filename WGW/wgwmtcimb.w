...
/*Program name     : Match Text file CIMB ON Phone                    */  
/*create by        : Ranu I. A62-0345  Date : 30/07/2019  Match file load and policy no CIMB */  
/*-------------------------------------------------------------------------------*/
DEF  stream  ns1.
...
DEFINE NEW SHARED WORKFILE wdetail2 NO-UNDO
    ...
    FIELD mobile        AS CHAR FORMAT "x(20)"  INIT ""
    FIELD body          AS CHAR FORMAT "x(50)"  INIT ""
    FIELD cashamt       AS CHAR FORMAT "x(15)"  INIT ""
    FIELD issue         AS CHAR FORMAT "x(15)"  INIT ""
    FIELD tax           AS CHAR FORMAT "x(20)"  INIT ""
    FIELD duty          AS CHAR FORMAT "x(20)"  INIT ""
    FIELD pretotal      AS CHAR FORMAT "x(20)"  INIT ""
    FIELD brname        AS CHAR FORMAT "x(20)"  INIT ""
    FIELD custype       AS CHAR FORMAT "x(20)"  INIT ""
    FIELD custbr        AS CHAR FORMAT "x(20)"  INIT ""
...
DEFINE VARIABLE rs_match AS INTEGER 
     ...
     RADIO-BUTTONS 
          "Match File Load To GW", 1,
"Match Policy and Update Data", 2
     SIZE 76 BY 1.52
...
DEFINE FRAME fr_main
     fi_remark AT ROW 9.52 COL 13.67 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     rs_match AT ROW 3.48 COL 4.17 NO-LABEL WIDGET-ID 2
     fi_filename AT ROW 5.43 COL 16 COLON-ALIGNED NO-LABEL
     fi_outfile AT ROW 6.71 COL 16 COLON-ALIGNED NO-LABEL
     bu_ok AT ROW 11.62 COL 61.83
     bu_exit AT ROW 11.62 COL 71.5
     bu_file AT ROW 5.43 COL 77.17
     fi_outfile2 AT ROW 7.95 COL 16 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     "Input File :" VIEW-AS TEXT
          SIZE 10.33 BY 1.05 AT ROW 5.43 COL 6.83
          BGCOLOR 19 FGCOLOR 2 FONT 6
     "File CIMB :" VIEW-AS TEXT
          SIZE 10.83 BY 1.05 AT ROW 6.71 COL 6.67
          BGCOLOR 19 FGCOLOR 1 FONT 6
     "  MATCH TEXT FILE CIMB ON PHONE" VIEW-AS TEXT
          SIZE 77 BY 1.67 AT ROW 1.48 COL 3.67
          BGCOLOR 3 FGCOLOR 7 FONT 2
     "Load PHONE :" VIEW-AS TEXT
          SIZE 14.33 BY 1.05 AT ROW 7.95 COL 3.5 WIDGET-ID 14
          BGCOLOR 19 FGCOLOR 1 FONT 6
...
DO:
    ...
    IF rs_match = 1 THEN ASSIGN fi_remark = "Remark : Match File Load use form Notify CIMB" .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  DEF  VAR  gv_prog    AS   CHAR.
      ...
      gv_prgid = "WGWMTCIMB.W".
  gv_prog  = "Match Text file Load CIMB and Update Policy no.".
...
/*********************************************************************/ 
  ...
  ASSIGN rs_match = 1
         fi_remark = "Remark : Match File Load Use form Notify CIMB" .
...
DO:
    ...
    ASSIGN  n_length   = 0      n_address  = ""
            ...
            n_tambon   = ""     n_amper    = ""          
            n_country  = ""     n_post     = ""
            n_address  = TRIM(wdetail2.pol_addr) + " " + /*AssetLocation  */  
                         trim(wdetail2.addr1) + " " +    /*AssetDistrict  */  
                         trim(wdetail2.addr2) + " " +    /*AssetZone      */  
                         trim(wdetail2.addr3) + " " +    /*AssetProvince  */  
                         trim(wdetail2.addr4)  .          /*ZipCode        */      
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_filecimb C-Win 
PROCEDURE proc_filecimb :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
EXPORT DELIMITER "|" 
    "ข้อมูลงาน CIMB รับประกันภัยทางโทรศัพท์ ." .
EXPORT DELIMITER "|" 
    "ลำดับที่"   
    "วันที่รับแจ้ง" 
    "เวลารับแจ้ง" 
...
FOR EACH wdetail2 WHERE wdetail2.polno <> "" AND wdetail2.comment = "COMPLETE" NO-LOCK.
...
DO:
    ...
    REPEAT:
        CREATE wdetail2.
        IMPORT DELIMITER "|"         
                wdetail2.RegisDate              /*BookingDate    */  
                wdetail2.polno                  /*PolicyNo       */  
                wdetail2.CMRName                /*Insurer        */  
                wdetail2.pol_title              /*Title          */  
                wdetail2.pol_fname              /*Name           */  
                wdetail2.pol_lname              /*Surname        */  
                wdetail2.icno                   /*idno           */  
                wdetail2.CustAddress            /*ContractNo     */  
                wdetail2.custtel                /*Telephone      */   
                wdetail2.mobile                 /*Mobile         */     
                wdetail2.brand                  /*BrandOfAuto    */  
                wdetail2.Brand_Model            /*Model          */  
                wdetail2.RegisNo                /*LicenseNo      */  
                wdetail2.RegisProv              /*ProvinceCode   */  
                wdetail2.chassis                /*ChassisNo      */  
                wdetail2.engine                 /*EngineNo       */  
                wdetail2.CC                     /*CC             */  
                wdetail2.body                   /*BodyType       */  
                wdetail2.yrmanu                 /*ModelYear      */  
                wdetail2.pol_addr1             /*AssetLocation  */  
                wdetail2.addr1                  /*AssetDistrict  */  
                wdetail2.addr2                  /*AssetZone      */  
                wdetail2.addr3                  /*AssetProvince  */  
                wdetail2.addr4                  /*ZipCode        */  
                wdetail2.cashamt                /*CashRecieptAmt */  
                wdetail2.totalprem              /*InsureAmt      */  
                wdetail2.SI                     /*SumInsureAmt   */  
                wdetail2.CovTyp                 /*TypeOfPolicy   */  
                wdetail2.issue                  /*IssuedDate     */  
                wdetail2.comdat                 /*EffectiveDate  */  
                wdetail2.expdat                 /*ExpireDate     */  
                wdetail2.netprem                /*NetPremium     */  
                wdetail2.tax                    /*Tax            */  
                wdetail2.duty                   /*Duty           */  
                wdetail2.pretotal               /*TotalPremium   */  
                wdetail2.safe_no                /*ReferenceNo    */  
                wdetail2.saleid                 /*MarketingCode  */  
                wdetail2.branch                 /*BranchCode     */  
                wdetail2.brname                 /*BranchName     */  
                wdetail2.custype                /*custype        */  
                wdetail2.custbr  .               /*custbranchno   */

       
        IF INDEX(wdetail2.RegisDate,"Trans")   <> 0 THEN  DELETE wdetail2.
        ELSE IF INDEX(wdetail2.RegisDate,"booking")   <> 0 THEN  DELETE wdetail2.
        ELSE IF INDEX(wdetail2.RegisDate,"summary")   <> 0 THEN  DELETE wdetail2.
        ELSE IF  wdetail2.RegisDate = "" THEN  DELETE wdetail2.
    END.  /* repeat  */
    RUN proc_matchfile.

    fi_remark =  "Export data To Report Match file Load CIMB." . 
    ...
    fi_remark =  "Match file Load complete ." . 
    DISP fi_remark WITH FRAME fr_main.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_matchfile C-Win 
PROCEDURE proc_matchfile :
DEF VAR n_branch AS CHAR .
DO:
    FOR EACH wdetail2 .
        IF      wdetail2.chassis = ""  THEN DELETE wdetail2.
        ELSE IF wdetail2.engine  = ""  THEN DELETE wdetail2.
        ELSE IF wdetail2.polno   <> "" THEN DO: 
        ...
        ELSE DO:
            ...
            FOR EACH  brstat.tlt USE-INDEX tlt06 WHERE 
                ...
                brstat.tlt.eng_no        = trim(wdetail2.engine)  and 
                brstat.tlt.genusr        = "Phone"                AND 
                brstat.tlt.lotno         = "CIMB"                .
                
                IF brstat.tlt.releas = "YES" THEN DO:   
                    ASSIGN wdetail2.comment = "มีการออกงานแล้ว Status " + tlt.releas.
                    NEXT.
                END.
                ELSE IF index(brstat.tlt.releas,"CA") <>  0 THEN DO:   
                     ASSIGN wdetail2.comment = "มีการยกเลิกข้อมูล Status = CA " .
                     NEXT.
                END.
                ELSE DO:
                    ASSIGN wdetail2.comment = ""
                           n_name    = ""       n_namefile  = ""    n_chk      = YES    
                           ...
                           n_namefile = TRIM((wdetail2.pol_title) + trim(wdetail2.pol_fname) + trim(wdetail2.pol_lname))  
                           n_namefile = TRIM(REPLACE(n_namefile," ",""))
                           n_producer = TRIM(brstat.tlt.comp_sub) .

                    /* เช็คชื่อในระบบ กับ ในไฟล์ */
                    IF SUBSTR(n_namefile,1,4) = "น.ส." THEN n_namefile = REPLACE(n_namefile,"น.ส.","นางสาว") .
                    ...
                    ELSE IF SUBSTR(n_namefile,1,3) = "นาย" THEN DO: 
                        ASSIGN  nv_title = "นาย" 
                                ...
                                wdetail2.NAME_insur = nv_title + " " + trim(wdetail2.NAME_insur).
                    END.
                    ELSE IF SUBSTR(n_namefile,1,4) = "น.ส." THEN DO: 
                        ASSIGN  nv_title = "นางสาว" 
                                wdetail2.NAME_insur = trim(SUBSTR(wdetail2.NAME_insur,5))
                                wdetail2.NAME_insur = nv_title + " " + trim(wdetail2.NAME_insur).
                    END.
                    ELSE DO:
                    ...
                    FIND LAST brstat.insure USE-INDEX insure03 WHERE 
                              brstat.insure.compno = "CIMB"   AND
                              brstat.insure.branch = TRIM(wdetail2.brname) NO-LOCK NO-ERROR .
                        ...
                        END.
                        ELSE DO:
                            MESSAGE "ไม่พบสาขา " + TRIM(wdetail2.branch) + " " + trim(wdetail2.brname) + " ในระบบพารามิเตอร์ของ CIMB " VIEW-AS ALERT-BOX.
                            ASSIGN wdetail2.comment = wdetail2.comment + "|" + "กรุณาตรวจสอบข้อมูลสาขาในไฟล์แจ้งงานอีกครั้ง".
                            NEXT.
                        END.

                    /* เช็คเบี้ย */
                    IF tlt.nor_grprm <> DECI(wdetail2.totalprem) AND ( tlt.nor_grprm > (DECI(wdetail2.totalprem) + 1 ))  OR
                       ( tlt.nor_grprm < (DECI(wdetail2.totalprem) - 1 ))  THEN DO:
                         ASSIGN wdetail2.comment = wdetail2.comment + "|" + "เบี้ยในไฟล์ " + (wdetail2.totalprem) + " และในระบบ " +
                                                   string(tlt.nor_grprm) + " ไม่ตรงกัน ".
                         NEXT.
                    END.
                    ...
                    IF brstat.tlt.policy <> "" THEN DO:
                                ...
                                IF brstat.tlt.policy = wdetail2.safe_no THEN DO: 
                                    ...
                                    IF NOT AVAIL sicuw.uwm100 THEN DO: 
                                        FIND LAST  bftlt  WHERE 
                                            bftlt.genusr        = "CIMB"  AND
                                            bftlt.nor_noti_ins  = trim(wdetail2.safe_no) NO-LOCK NO-ERROR NO-WAIT.
                                        IF NOT AVAIL bftlt THEN DO:  
                                            ASSIGN wdetail2.polno  = brstat.tlt.policy
                                                   wdetail2.comment  = "COMPLETE"
                                                   brstat.tlt.endcnt = 1. /* status load */
                                        END.
                                        ELSE DO:
                                            ASSIGN wdetail2.comment = wdetail2.comment + "|" + wdetail2.safe_no + " มีข้อมูลในถังพักของ CIMB แล้ว".
                                            NEXT.
                                        END.
                                    END.
                                    ELSE DO:
                        ...
                        ELSE DO:
                             FIND LAST  bftlt  WHERE 
                               ...
                               IF NOT AVAIL bftlt THEN DO:  
                                   ASSIGN wdetail2.polno  = trim(bftlt.nor_noti_tlt) . /* status load */
                               END.
                               ELSE DO:
                                   ASSIGN wdetail2.comment = wdetail2.comment + "|" + wdetail2.safe_no + " มีข้อมูลในถังพักของ CIMB Status Release แล้ว".
                                   NEXT.
                               END.
                        END.
                    END.
                    ...
                    END.
                    ASSIGN wdetail2.comment = "COMPLETE" .
                    IF wdetail2.polno <> "" AND wdetail2.comment = "COMPLETE" THEN DO: 
                        RUN proc_updatetlt.
                    END.
                END.
...
------------------------------------------------------------------------------*/
DO:
    fi_remark =  "Update tlt... " +  wdetail2.polno . 
    DISP fi_remark WITH FRAME fr_main.

    ASSIGN  nv_notify = "" 
            nv_notify = "BookingDate " + trim(wdetail2.RegisDate) .
    RUN proc_addr.
    ASSIGN 
        tlt.nor_usr_ins   =   trim(wdetail2.saleid)   /*  ผู้แจ้ง*/
        tlt.comp_noti_tlt =   TRIM(wdetail2.CustAddress)   /*  refno  */
      
       tlt.nor_noti_ins  = IF n_brsty = "M"  THEN tlt.nor_noti_ins ELSE trim(wdetail2.brname) 
       tlt.nor_usr_tlt   = IF n_brsty = "M"  THEN tlt.nor_usr_tlt  ELSE TRIM(wdetail2.branch)
       tlt.colorcod      = IF n_brsty = "M" THEN tlt.colorcod ELSE TRIM(n_brsty) 
       ...
       tlt.endcnt        = 1
       wdetail2.comdat   = string(tlt.nor_effdat,"99/99/9999")          
       wdetail2.expdat   = string(tlt.expodat,"99/99/9999") 
       wdetail2.netprem  = tlt.dri_name2
       wdetail2.tax      = string(Truncate(DECI(tlt.dri_name2) * 0.4 / 100,0))
       wdetail2.duty     = string(DECI(tlt.dri_name2) * 7 / 100 )
       wdetail2.pretotal = STRING(DECI(wdetail2.netprem) + deci(wdetail2.tax) + deci(wdetail2.duty)) . 
    ...
    IF AVAIL sicuw.uwm100 THEN DO: 
        ...
        ELSE DO:
            FIND LAST brstat.tlt USE-INDEX tlt06 WHERE 
                ...
                brstat.tlt.genusr        =  "phone"                   AND 
                brstat.tlt.lotno         =  "CIMB"                   NO-ERROR NO-WAIT.

...
/*programname : Load text file Nissan to GW  (ต่ออายุ)                              */ 
/* Copyright  : Safety Insurance Public Company Limited                   */
/*copy write  : wgwargen.w                                                */ 
/*              บริษัท ประกันคุ้มภัย จำกัด (มหาชน)                        */
/*create by   : Ranu i. A60-0139  date. 10/04/2017                  
                นำเข้า text file Nissan to GW system งานต่ออายุ             */
/***************************************************************************/
/*Modify By : Ranu I. a60-0272 Date: 23/06/2017 เพิ่มการเก็บข้อมูลช่อง icno */
   ...
   /* ASSIGN 
    ...
    IF wdetail.compol <> "" AND wdetail.poltyp = "V72" THEN DO:
        ...
        IF AVAIL brstat.tlt THEN DO:
            IF brstat.tlt.rec_addr5 <> "" THEN ASSIGN wdetail.comment = "มีเบอร์พรบ.ใหม่แล้ว " + brstat.tlt.rec_addr5.
            ELSE DO:
                ASSIGN 
                ...
                IF INDEX(brstat.tlt.releas,"YES") <> 0 THEN 
                    ASSIGN wdetail.comment = IF wdetail.comment = "" THEN "มีการออกกรมธรรม์แล้ว " +  brstat.tlt.rec_addr5
                                             ELSE  wdetail.comment  + "/ " + "มีการออกกรมธรรม์แล้ว " +  brstat.tlt.rec_addr5.
    ...
    IF wdetail.policy <> "" OR wdetail.compol <> "" THEN DO:
        FIND LAST brstat.tlt USE-INDEX tlt06  WHERE         
         ...
         IF AVAIL brstat.tlt THEN DO:
             IF brstat.tlt.policy <> "" THEN ASSIGN wdetail.comment = "มีเบอร์กรมธรรม์ใหม่แล้ว " + brstat.tlt.policy.
             IF brstat.tlt.rec_addr5 <> "" THEN ASSIGN wdetail.comment = "มีเบอร์ พรบ.ใหม่แล้ว " + brstat.tlt.rec_addr5.
             IF INDEX(brstat.tlt.releas,"YES") <> 0 THEN 
                    ASSIGN wdetail.comment = IF wdetail.comment = "" THEN "มีการออกกรมธรรม์แล้ว " +  brstat.tlt.policy
                                             ELSE  wdetail.comment  + "/ " + "มีการออกกรมธรรม์แล้ว " +  brstat.tlt.policy.
             ELSE DO:
                    ASSIGN 
...
IF  CONNECTED("sic_exp") THEN DO:
    ...
    IF wdetail.cr_2 <> "" THEN DO: /* ใส่ข้อมูลให้ พรบ. */
        FIND LAST wdetail WHERE wdetail.policy = n_cr2  AND wdetail.covcod = "T" NO-LOCK no-error .
            ...
            RELEASE wdetail.
    END.
    FIND LAST wdetail WHERE wdetail.policy = n_pol AND wdetail.covcod <> "T" NO-LOCK NO-ERROR . /* ใส่ข้อมูลให้ กธ. */
    IF AVAIL wdetail THEN DO:
             IF DECI(wdetail.si) <> DECI(WDETAIL.deductpd) THEN DO:
                MESSAGE "ทุนประกันในไฟล์งาน กับในระบบไม่ตรงกัน !! " VIEW-AS ALERT-BOX.
                ASSIGN wdetail.redbook   = trim(re_redbook)                                            
                       /*wdetail.brand     = trim(re_brand)*/                                            
                       /*wdetail.model     = trim(re_model)*/                                           
                       wdetail.caryear   = trim(re_caryear)                                            
                       ...
                       wdetail.chasno    = trim(re_chasno)                                             
                       wdetail.eng       = trim(re_eng)
                       wdetail.WARNING   = IF wdetail.WARNING <> "" THEN wdetail.WARNING + "|ทุนประกันในไฟล์งาน กับในระบบไม่ตรงกัน !! " 
                                           ELSE "ทุนประกันในไฟล์งาน กับในระบบไม่ตรงกัน !! " .  
             END.
             ELSE DO:
    ...
    ASSIGN               
       ...
       /*nv_cstflg  = "C"  /*C=CC ; S=Seat ; T=Tons ; W=Watts*/ 
        ...
        /*nv_supe    = NO*/                                              
         ...
         /*nv_status  = "" */
         nv_fcctv   = NO .
     IF wdetail.prepol <> "" THEN nv_netprem  = DECI(wdetail.premt). /*มาจากใบเตือน*/
...
REPEAT: 
    ...
    IMPORT DELIMITER "|" 
       ...
       /* wdetail.poltyp  */         /*Type            */ 
        ...
        Wdetail.n_branch         /*BR              */ 
        Wdetail.subclass  .       /*class           */ 
    IF        INDEX(wdetail.icno,"ลำดับ")   <> 0  THEN DELETE wdetail.
...
------------------------------------------------------------------------------*/
/* comment by :A64-0138...
IF wdetail.premt <> "" THEN DO:  /*กรณี ต้องการ ให้ได้เบี้ย ตามไฟล์ */

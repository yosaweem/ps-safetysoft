...
/* Modify By : Porntiwa T. A62-0105  Date : 18/07/2019
             : Change Host => TMSth */
/* Modify by : Ranu I. A63-0164 แก้ไข pack T และงานต่ออายุใช้ Pack ตามพารามิเตอร์ */
/* Modify by : Kridtiya i. A63-0419 เพิ่ม การเช็ค susspect                        */
...
/*MOdify by  : Kritapoj S. A65-0372 Date 16/01/2023 เพิ่มช่องcolor และในช่องcolor ออกเป็นรหัสสี*/
/*---------------------------------------------------------------------*/
{wgw\wgwlicbc.i}      /*ประกาศตัวแปร*/
...
IF NOT AVAIL wdetail  THEN DO:
    ...
    ASSIGN 
        ...
        /*wdetail.np_name2   = IF INDEX(ns_namepol,ns_cname) <> 0 THEN "" ELSE "และ/หรือ " + TRIM(ns_namepol)*/ /*A60-0263*/
        ...
        /* end A60-0263 */
        wdetail.benname    = IF INDEX(ns_assured,"ไม่มี") <> 0 THEN "" ELSE IF INDEX(ns_assured,"ICBC") <> 0 THEN "บริษัทลิสซิ่งไอซีบีซี (ไทย) จำกัด" 
       ...
       /* wdetail.agent      = trim(ns_agent)       */ /*A62-0082*/
        ...
        IF index(wdetail.namerequest,"นาย") <> 0 THEN DO:
        ...
        END.
        ELSE IF index(wdetail.namerequest,"นาง") <> 0 THEN DO:
            ASSIGN wdetail.promo   = "ICBC " + IF index(wdetail.namerequest," ") <> 0 THEN SUBSTR(trim(wdetail.namerequest),4,index(wdetail.namerequest," ")) 
                                               ELSE SUBSTR(trim(wdetail.namerequest),4,LENGTH(wdetail.namerequest)). 
        END.
        ELSE IF index(wdetail.namerequest,"น.ส.") <> 0 THEN DO:
        ...
        END.
        ELSE IF index(wdetail.namerequest,"นางสาว") <> 0 THEN DO:
        ...
        END.
        ELSE IF index(wdetail.namerequest,"คุณ") <> 0 THEN DO:
            ASSIGN wdetail.promo   = "ICBC " + IF index(wdetail.namerequest," ") <> 0 THEN SUBSTR(trim(wdetail.namerequest),4,index(wdetail.namerequest," ")) 
                                               ELSE SUBSTR(trim(wdetail.namerequest),4,LENGTH(wdetail.namerequest)).
        END.
...
DO TRANSACTION:
   ASSIGN
      ...
      sic_bran.uwm100.name2  = trim(wdetail.np_name2)              /*wdetail.receipt_name*/        /*และ/หรือ */              
...
/*IF  (wdetail.namerequest <> "") OR (wdetail.daterequest <> "") OR (wdetail.nocheck <> "") THEN DO:*/
   /* DO WHILE nv_line1 <= 6 :*/ /*a60-0263*/
    DO WHILE nv_line1 <= 10 :  /*a60-0263*/
        ...
        IF nv_line1 = 2 THEN wuppertxt3.txt = "วันที่แจ้งงาน : "    + SUBSTR(wdetail.daterequest,7,2) + "/"
                                                                    + SUBSTR(wdetail.daterequest,5,2) + "/"
                                                                    + SUBSTR(wdetail.daterequest,1,4) + 
                                              "  ชื่อผู้แจ้งงาน : " + wdetail.namerequest . 
        IF nv_line1 = 3 THEN wuppertxt3.txt = "เลขที่รับแจ้ง : "    + wdetail.notify_no .
        IF nv_line1 = 4 THEN wuppertxt3.txt = "วันที่ยืนยัน : "     + substr(wdetail.nocheck,7,2) + "/"
                                                                    + substr(wdetail.nocheck,5,2) + "/" 
                                                                    + substr(wdetail.nocheck,1,4) +  
                                            "  ชื่อผู้ยืนยัน : "    + wdetail.notfyby  .              
        IF nv_line1 = 5 THEN wuppertxt3.txt = "เลขตรวจสภาพ : "      + wdetail.txtmemo  . 
        IF nv_line1 = 6 THEN wuppertxt3.txt = "หมายเหตุ : "         + wdetail.txtmemo2 .
        IF nv_line1 = 7 THEN wuppertxt3.txt = "แคมเปญ : "           + wdetail.campaign .  /*A60-0263*/

...
/*Modify by    : Kridtiya i. A64-0137 เพิ่มการรับค่า รหัส Dealer code จากใบเตือน ให้กรมธรรม์ใหม่ */
...
DEFINE INPUT-OUTPUT  PARAMETER  np_prempa      AS CHAR FORMAT "x(1)"    INIT "".       
...
DEFINE INPUT-OUTPUT  PARAMETER  np_weight      AS CHAR FORMAT "x(30)"   INIT "". .                      
...
DEFINE INPUT-OUTPUT  PARAMETER  np_benname     AS CHAR FORMAT "x(60)"   INIT "". 
DEFINE INPUT-OUTPUT  PARAMETER  np_vehreg      AS CHAR FORMAT "x(11)"   INIT "".              
DEFINE INPUT-OUTPUT  PARAMETER  np_uom1_v      AS DECI INIT 0.                               
DEFINE INPUT-OUTPUT  PARAMETER  np_uom2_v      AS DECI INIT 0.                                   
DEFINE INPUT-OUTPUT  PARAMETER  np_uom5_v      AS DECI INIT 0.                                  
DEFINE INPUT-OUTPUT  PARAMETER  np_si          AS CHAR FORMAT "x(30)" INIT "".              
DEFINE INPUT-OUTPUT  PARAMETER  np_fi          AS CHAR FORMAT "x(30)" INIT "".             
DEFINE INPUT-OUTPUT  PARAMETER  np_baseprm     AS DECI INIT 0.            
DEFINE INPUT-OUTPUT  PARAMETER  np_41          AS DECI INIT 0.            
DEFINE INPUT-OUTPUT  PARAMETER  np_42          AS DECI INIT 0.            
DEFINE INPUT-OUTPUT  PARAMETER  np_43          AS DECI INIT 0.            
DEFINE INPUT-OUTPUT  PARAMETER  np_seat41      AS DECI INIT 0.         
...
DEFINE INPUT-OUTPUT  PARAMETER  np_dealer      AS CHAR FORMAT "x(20)"   INIT "".   /*A64-0137*/           
...
IF AVAILABLE sic_exp.uwm100 THEN DO:
    ASSIGN np_insref = sic_exp.uwm100.insref
           np_dealer = sic_exp.uwm100.finint .    /*A64-0137*/ 
    FIND sic_exp.uwm130 USE-INDEX uwm13001 WHERE
    ...
    IF AVAIL sic_exp.uwm120 THEN
        ...
        np_prempa   = substring(sic_exp.uwm120.class,1,1)
        np_subclass = substring(sic_exp.uwm120.class,2,3).
    ...
    IF AVAIL sic_exp.uwm301 THEN
        ASSIGN 
        np_weight  = string(sic_exp.uwm301.Tons)     
    ...
    FOR EACH sic_exp.uwd132 USE-INDEX uwd13290  WHERE
        ...
        ELSE IF sic_exp.uwd132.bencod                = "base" THEN
            ASSIGN   np_baseprm = DECI(SUBSTRING(sic_exp.uwd132.benvar,31,30)).

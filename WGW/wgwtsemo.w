...
/*Modify by : Kridtiya i. A57-0088 date. 10/03/2014 เพิ่มการเก็บข้อมูล Redbook,year,MarketPrice */                        
CREATE WIDGET-POOL.
...
DEFINE   WORKFILE wdetail 
    ...
    FIELD  brand    AS CHAR FORMAT "x(30)"  INIT ""   /*A57-0017*/
    FIELD  model    AS CHAR FORMAT "x(60)"  INIT ""   /*A57-0017*/
    FIELD  n_redbook       AS CHAR FORMAT "x(10)" INIT ""    /*A57-0088*/  
    FIELD  n_year          AS CHAR FORMAT "x(4)"  INIT ""    /*A57-0088*/  
    FIELD  n_marketPrice   AS CHAR FORMAT "x(20)" INIT "" .  /*A57-0088*/ 
...
DEF VAR n_brand  AS CHAR FORMAT "x(30)"  INIT "" .  /*A57-0017*/
DEF VAR n_model  AS CHAR FORMAT "x(60)"  INIT "" .  /*A57-0017*/
DEF VAR n_countload     AS DECI FORMAT "->>>>>9"  INIT "" .  /*A57-0017*/
DEF VAR n_countcomplete AS DECI FORMAT "->>>>>9"  INIT "" .  /*A57-0017*/
DEF VAR n_redbook       AS CHAR FORMAT "x(10)"    INIT "" .  /*A57-0088*/
DEF VAR n_year          AS CHAR FORMAT "x(4)"     INIT "" .  /*A57-0088*/
DEF VAR n_marketPrice   AS CHAR FORMAT "x(20)"    INIT "" .  /*A57-0088*/
...
Insure.VatCode Insure.FName Insure.LName Insure.Text5 Insure.Text3 ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brInsure WGWTSEMO _STRUCTURED
  QUERY brInsure NO-LOCK DISPLAY
      Insure.CompNo COLUMN-LABEL "Code" FORMAT "X(13)":U WIDTH 8
      Insure.InsNo COLUMN-LABEL "ID" FORMAT "X(8)":U
      Insure.VatCode COLUMN-LABEL "Vat_Co." FORMAT "X(10)":U
      Insure.FName COLUMN-LABEL "Deler Name." FORMAT "X(55)":U
            WIDTH 35
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW WGWTSEMO ASSIGN
         HIDDEN             = YES
         TITLE              = "Set Company Code Delar [WGWTSEMO.W]"
         HEIGHT             = 24.05
         WIDTH              = 132
         MAX-HEIGHT         = 33.71
         MAX-WIDTH          = 170.67
...
"Insure.CompNo" "Code" "X(13)" "character" ? ? ? ? ? ? no ? no no "8" yes no no "U" "" ""
     _FldNameList[2]   > stat.Insure.InsNo
"Insure.InsNo" "ID" "X(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
"Insure.FName" "Deler Name." "X(55)" "character" ? ? ? ? ? ? no ? no no "35" yes no no "U" "" ""
     _FldNameList[5]   > stat.Insure.LName
"Insure.LName" "show room" "X(30)" "character" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" ""
     _FldNameList[6]   > stat.Insure.Text5
...
ON END-ERROR OF WGWTSEMO /* Set Company Code Delar [WGWTSEMO.W] */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL WGWTSEMO WGWTSEMO
ON WINDOW-CLOSE OF WGWTSEMO /* Set Company Code Delar [WGWTSEMO.W] */
DO:

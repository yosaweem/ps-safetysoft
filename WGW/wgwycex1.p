...
/* WRITE  BY   : Kridtiya i.A53-0232  05/08/2010                            */
/*             : ส่วนการค้นหาค่า งานต่ออายุเพื่อให้ค่า กรมธรรม์ต่ออายุใหม่  */
/* modify BY   : Kridtiya i.A54-0129  26/05/2011                            */
/*             : ส่วนการค้นหาค่า งานต่ออายุเพื่อให้ค่า ข้อมูลลูกค้าชื่อเดิม */
/* modify BY   : Kridtiya i.A55-0182  21/01/2013                           */
/*             : ส่วนการให้ค่ายี่ห้อรถและ รุ่นรถ */
...
DEFINE INPUT-OUTPUT  PARAMETER n_icno       AS CHAR FORMAT "x(15)"  .
...
IF AVAILABLE sic_exp.uwm100 THEN DO:
    ...
    IF AVAIL sic_exp.uwm120 THEN
        ...
        n_icno     = TRIM(sic_exp.uwm100.anam2)
    ...
    IF AVAIL uwm301 THEN
        ...
        n_model    = trim(substr(sic_exp.uwm301.moddes,INDEX(sic_exp.uwm301.moddes," ") - 1))*/
        n_brand    = trim(sic_exp.uwm301.moddes)
        n_model    = ""
    ...
    FOR EACH sic_exp.uwd132 USE-INDEX uwd13290  WHERE
        ...
        ELSE IF index(sic_exp.uwd132.bencod,"CL") <> 0 THEN
            /*ASSIGN nv_flet_per = DECIMAL(SUBSTRING(sic_exp.uwd132.benvar,31,30)).*//*A57-0042*/
            ASSIGN nv_cl_per = DECIMAL(SUBSTRING(sic_exp.uwd132.benvar,31,30)).      /*A57-0042*/

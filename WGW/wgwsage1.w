...
/*programname : load text file บจก.สยามออโต้เซอร์วิส to GW (งานนิว)     */ 
...
/*modify by   : kridtiya i. A56-0018 เพิ่มการเลือกรับ เทค F15,F17 ที่หน้าจอ                */
/*modify by   : kridtiya i. A56-0045 เพิ่มการรับค่าลักษณการใช้รถจากไฟล์นำเข้า              */
...
/*modify by   : Kridtiya i. A57-0141 date 22/04/2014 เพิ่มการบันทึกข้อความให้ได้มากกว่า14บรรทัด*/
/*modify by   : Kridtiya i. A57-0370 date 11/10/2014 เพิ่มการรับข้อมูลbody,Garage จากกรมต้นแบบ*/
/*Modify by   : Ranu I. A58-0354  Date 23/09/2015  เพิ่มการรับข้อมูล Promotion จาก กธ.ตัวอย่าง */
/*Modify by   : Kridtiya i. A59-0186 date: 30/05/2016 add occupation                    */
/*Modify by   : Ranu I. A60-0163  ให้ดึงข้อมูลรุ่นรถจากไฟล์  */
/*modify by   : Ranu I. A61-0262 ให้ดึงข้อมูลจาก policy master และแก้ไขไฟล์โหลด */
/*modify by   : Saowapa U. A62-0128 21/03/2018 เพิ่มช่อง เลขสติ๊กเกอร์ และ Cpol  ในไฟล์โหลด ต่อจากเบอร์กรมธรรม์ 72   ช่อง  Cpol   เก็บข้อมูลในช่อง C.Pol.no 
                เพิ่มเติมปรับปรุงโปรแกรม Load งาน Fleet กลุ่มคาร์เรนท์  เฉพาะ พ.ร.บ.  ที่สามารถดึง Body และ CC. จาก Policy Master ในการ Load Text 
                แก้ไข  Load Text  กลุ่มคาร์เรนท์  ให้อ้างอิง Policy Master ทัง Insure Code, Producer Code , Agent Code ,Dealer code  และ Vat Code
                เพิ่มการดึงข้อมูล ช่อง Prom.. จาก policy Master*/
...
DEFINE FRAME fr_main
     ...
     "Batch No. :" VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "     IMPORT TEXT FILE MOTOR [CARRENT GROUP - งานใหม่/ ต่ออายุ ]" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW c-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Import Text file บจก.สยามคาร์เรนท์ NEW"
         HEIGHT             = 24.05
         WIDTH              = 133.17
         MAX-HEIGHT         = 45.76
         MAX-WIDTH          = 213.33
...
ON END-ERROR OF c-Win /* Import Text file บจก.สยามคาร์เรนท์ NEW */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-Win c-Win
ON WINDOW-CLOSE OF c-Win /* Import Text file บจก.สยามคาร์เรนท์ NEW */
DO:
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "Wgwsage1.w".
  gv_prog  = "Load Text & Generate บจก.สยามออโต้เซอร์วิส(NEW)".
  fi_loaddat = TODAY.
  DISP fi_loaddat WITH FRAME fr_main.
...
IF wdetail.policy <> "" THEN DO:
   ...
   IF AVAIL sicuw.uwm100 THEN
        ASSIGN /*a490166*/
            wdetail.comment = wdetail.comment + "| มีเบอร์กรมธรรม์ " + wdetail.policy + " ในระบบพรีเมียมแล้ว " 
            wdetail.pass    = "N"
            wdetail.OK_GEN  = "N". 
...
DO:
    ...
    REPEAT:
        ...
        IF      index(n_idno,"ที่") <> 0 THEN RUN proc_initdata.
        ELSE IF index(n_idno,"เลข") <> 0 THEN RUN proc_initdata.
...
/*     wdetail.premt = string(nv_pdprm) */
    ...
    nv_gapprem    = nv_gapprm   . /*เบี้ยเต็มปี*/
    ...
    IF sic_bran.uwm100.expdat NE ? AND sic_bran.uwm100.comdat NE ? THEN DO:
        ...
        ELSE DO:
         ...
         /* nv_netprem  = TRUNCATE(DECI(wdetail.premt) / nv_polday * 365 ,0 ) .
          nv_pdprem   = DECI(wdetail.premt) .  รับจาก Polmaster */
/*
...
IF NOT AVAIL wdetail THEN DO:
    ...
    IF fi_policymas <> ""   THEN DO:
        /*-- A58-0354 : ใส่ข้อมูล Promotion ตามกรมธรรม์ตัวอย่าง --*/
   ...
   /* IF ra_f6text = 2 THEN DO:  A61-0262 */ /*  ตามกรมธรรม์ หลัก ...*/

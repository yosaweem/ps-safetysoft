...
/*Modify by : Ranu I. A64-0092 เพิ่มเงื่อนไขงาน Group Ford และการเก็บอุปกรณ์เสริม */
/*Modify by : Ranu I. A64-0138 28/06/2021 เพิ่มเงื่อนไขงานการเช็ค product promo */
...
/*Modify by   : Ranu I. A67-0087 เพิ่มเงื่อนไขรถไฟฟ้า */
/*Modify by   : Ranu I. A67-0144 แก้ไข Format file Load แก้ไขการเช็ค CC, อุปกรณ์เสริม , promo ,Product */
...
DEFINE FRAME fr_main
     ...
     "    Previous Batch No. :" VIEW-AS TEXT
          ...
          BGCOLOR 18 FGCOLOR 1 FONT 6
     "    IMPORT TEXT FILE MOTOR [OEM-FORD-ป้ายแดง/ต่ออายุ ]" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW c-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Load Text file OEM-FORD - ป้ายแดง/ต่ออายุ"
         HEIGHT             = 24.1
         WIDTH              = 133.17
         MAX-HEIGHT         = 45.76
         MAX-WIDTH          = 213.33
...
ON END-ERROR OF c-Win /* Load Text file OEM-FORD - ป้ายแดง/ต่ออายุ */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-Win c-Win
ON WINDOW-CLOSE OF c-Win /* Load Text file OEM-FORD - ป้ายแดง/ต่ออายุ */
DO:
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "Wgwtfgen".
  gv_prog  = "Load Text & Generate OEM-FORD [ป้ายแดง/ต่ออายุ]".
  fi_loaddat = TODAY.
  DISP fi_loaddat WITH FRAME fr_main.
  ...
  ASSIGN
      ...
      fi_textf17   = "TISCO_F7"
      fi_camp      = "PACK_TISCO"
      fi_process   = "Load Text File OEM-FORD [งานป้ายแดง...]" .
      RUN proc_protis.  
      RUN proc_insuricno.
...
IF INDEX(wdetail.remark,"FE2+") <> 0 THEN DO: 
    ASSIGN 
        ...
        /*SUBSTRING(sic_bran.uwm301.prmtxt,181,60) =  "Campaign: C63/00116 (ป.2+)" */ /*A64-0092*/
...
ELSE do:
    ...
    END.
    ELSE ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   =  "คุ้มครองอุปกรณ์ตกแต่งเพิ่มเติมไม่เกิน 20,000 บาท" .
    /* end A64-0092*/
    /* comment by A64-0092..
    IF wdetail.acc <> "" THEN DO:
       ASSIGN 
...
ELSE do:
    ...
    ELSE IF wdetail.covcod = "1" THEN DO: 
        IF trim(wdetail.brand) = "NISSAN" THEN ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)  =  "คุ้มครองอุปกรณ์ตกแต่งเพิ่มเติมไม่เกิน 30,000 บาท" .
        ELSE IF TRIM(wdetail.brand) = "FORD" OR TRIM(wdetail.brand) = "ORA" OR TRIM(wdetail.brand) = "HAVAL" THEN DO:
            IF (YEAR(TODAY) - INT(wdetail.caryear) + 1 ) < 8 THEN DO:
                ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   =   "ความคุ้มครองโจรกรรมทรัพย์สินส่วนบุคคลภายในรถ 20,000 บาท" 
                       SUBSTRING(sic_bran.uwm301.prmtxt,61,60)  =   "คุ้มครองอุปกรณ์ตกแต่งเพิ่มเติมไม่เกิน 20,000 บาท" .
            END.
            ELSE  ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)  =  "คุ้มครองอุปกรณ์ตกแต่งเพิ่มเติมไม่เกิน 20,000 บาท" .
        END.
    END. /* end : A65-0035 */
        ...
        FIND FIRST stat.maktab_fil USE-INDEX maktab01  WHERE 
         ...
         ELSE DO:
             FIND FIRST sicsyac.xmm102 USE-INDEX xmm10201 WHERE 
              ...
              ELSE DO:
                  ASSIGN wdetail.comment = wdetail.comment + "|ไม่พบ Redbook Code " +  wdetail.redbook  + " ของ Class " + n_sclass72
                         wdetail.redbook = "" .
              END.
         END.
    END. 
...
FOR EACH wdetail WHERE wdetail.policy  NE " "  .
   ...
   /*A54-0141 */
    ...
    ELSE IF LENGTH(wdetail.addr1) > 45 THEN DO:  /*moo soy road*/
        ...
        END.      /*end  road..*/
        ELSE IF INDEX(wdetail.addr1,"ซ.") <> 0  THEN DO:
            ASSIGN 
            wdetail.soy     = trim(SUBSTR(wdetail.addr1,r-INDEX(wdetail.addr1,"ซ.")))
            wdetail.addr1   = trim(SUBSTR(wdetail.addr1,1,r-INDEX(wdetail.addr1,"ซ.") - 1 )) 
        ...
        END.
        ELSE IF INDEX(wdetail.addr1,"ซอย") <> 0 THEN DO:
            ASSIGN 
            wdetail.soy     = trim(SUBSTR(wdetail.addr1,r-INDEX(wdetail.addr1,"ซอย")))
            wdetail.addr1   = trim(SUBSTR(wdetail.addr1,1,r-INDEX(wdetail.addr1,"ซอย") - 1 ))
            wdetail.tambon  = trim(wdetail.soy) + " " + trim(wdetail.tambon) .
            ...
            /*wdetail.country = "" .                                   */  /*A64-0138*/
        END.
        ELSE IF R-INDEX(wdetail.addr1,"หมู่") <> 0  THEN DO:
        ...
        IF LENGTH(wdetail.amper) > 35 THEN DO:
            IF r-INDEX(wdetail.amper,"อ.") <> 0 THEN
                ASSIGN 
                wdetail.country = SUBSTR(wdetail.amper,r-INDEX(wdetail.amper,"อ.")) + " " + wdetail.country
                wdetail.amper   = SUBSTR(wdetail.amper,1,r-INDEX(wdetail.amper,"อ.") - 1).
            ELSE IF r-INDEX(wdetail.amper,"อำเภอ") <> 0 THEN
                ASSIGN 
                wdetail.country = SUBSTR(wdetail.amper,r-INDEX(wdetail.amper,"อำเภอ")) + " " + wdetail.country
                wdetail.amper   = SUBSTR(wdetail.amper,1,r-INDEX(wdetail.amper,"อำเภอ") - 1).
            ELSE IF r-INDEX(wdetail.amper,"เขต") <> 0 THEN
                ASSIGN 
                wdetail.country = SUBSTR(wdetail.amper,r-INDEX(wdetail.amper,"เขต")) + " " + wdetail.country
                wdetail.amper   = SUBSTR(wdetail.amper,1,r-INDEX(wdetail.amper,"เขต") - 1).
...
DO:
    IF INDEX(wdetail.remark,"FE2+") <> 0 THEN DO: 
        IF (YEAR(TODAY) - INT(wdetail.caryear) + 1 ) <= 7 AND wdetail.garage = "G" THEN DO: /* A67-0087*/
            IF wdetail.acc = ""  THEN DO:
                 ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   = "MT001_PB คุ้มครองการประกันภัยโจรกรรมทรัพย์สินส่วนบุคคล"      
                        SUBSTRING(sic_bran.uwm301.prmtxt,61,60)  = "ที่อยู่ภายในรถยนต์ 20,000 บาท"                              
                        SUBSTRING(sic_bran.uwm301.prmtxt,121,60) = "MT002_FL คุ้มครองการประกันภัยคุ้มครองภัยน้ำท่วมตามทุนประกัน"
                        SUBSTRING(sic_bran.uwm301.prmtxt,181,60) = "คุ้มครองอุปกรณ์เสริมเพิ่มเติมไม่เกิน 5% ของทุนประกันภัย" .
            END.
            ELSE DO:
                ASSIGN 
                    SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   =  "MT001_PB คุ้มครองการประกันภัยโจรกรรมทรัพย์สินส่วนบุคคล"
        ...
        ELSE DO:
            IF wdetail.acc = "" AND wdetail.garage = "G" THEN DO:
                 ASSIGN SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   = "คุ้มครองอุปกรณ์เสริมเพิ่มเติมไม่เกิน 5% ของทุนประกันภัย" . 
            END.
            ELSE DO:
                ASSIGN 
                    SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   = SUBSTRING(wdetail.acc,1,60)   
    ...
    ELSE DO:
        /*IF  INDEX(wdetail.brand,"HAVAL")  <> 0 OR INDEX(wdetail.brand,"ORA") <> 0 THEN DO:   /*A66-0070*/*/ /* A67-0087*/
         IF INDEX(wdetail.brand,"HAVAL")  <> 0 OR INDEX(wdetail.brand,"ORA") <> 0 OR INDEX(wdetail.brand,"TANK") <> 0 THEN DO:   /* A67-0087*/
            ...
            END.
            ELSE DO:
                IF wdetail.acc = "" THEN ASSIGN  SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   = "คุ้มครองอุปกรณ์เสริมเพิ่มเติมไม่เกิน 50,000 บาทรวมในทุน". 
                ELSE DO:  /*มีอุปกรณ์*/
                    ASSIGN 
                        SUBSTRING(sic_bran.uwm301.prmtxt,1,60)   = SUBSTRING(wdetail.acc,1,60)      
                        SUBSTRING(sic_bran.uwm301.prmtxt,61,60)  = SUBSTRING(wdetail.acc,61,60)     
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
/* comment by : A65-0035 ไม่ได้เรียกใช้งาน....
IF wdetail.model = "commuter" THEN wdetail.seat = 12.
IF (wdetail.si = "") OR (wdetail.si = "0.00") OR (wdetail.si = "0" ) THEN DO:
    Find LAST stat.maktab_fil Use-index      maktab04           Where
        stat.maktab_fil.makdes   =    wdetail.brand             And                  
...
DO WHILE nv_line1 <= 12:      /*A67-0087*/
   ...
   /* IF nv_line1 = 3  THEN wuppertxt3.txt = "Notify time:" + wdetail.enttim   .*/
    ...
    /* add by : A67-0087 ..*/
    IF nv_line1 = 8  THEN wuppertxt3.txt = "สีรถ :" + wdetail.colorcode. 
    IF nv_line1 = 9  THEN wuppertxt3.txt = "หมายเลขเครื่องยนต์ 3 :" + wdetail.engno3 . 
    IF nv_line1 = 10  THEN wuppertxt3.txt = "หมายเลขเครื่องยนต์ 4 :" + wdetail.engno4 . 
    IF nv_line1 = 11  THEN wuppertxt3.txt = "หมายเลขเครื่องยนต์ 5 :" + wdetail.engno5 . 
    /* end : A67-0087 */
    /*--end A60-0118--*/
    nv_line1 = nv_line1 + 1.                                                   
END.

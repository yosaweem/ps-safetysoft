...
/*---A64-0380---*/
DEF VAR nv_output_newmot  AS CHAR FORMAT "X(250)" INIT "".
DEF VAR nv_output_newnon  AS CHAR FORMAT "X(25)" INIT "".
DEF VAR nv_output_newsummot  AS CHAR FORMAT "X(25)" INIT "".
DEF VAR nv_output_newsumnon  AS CHAR FORMAT "X(250)" INIT "".
...
DEF OUTPUT PARAMETER oumot_sta AS CHAR FORMAT "X(6)".
...
DEFINE FRAME fr_main
     ...
     "Earn / Unearn Premium Report - Motor" VIEW-AS TEXT
          SIZE 37.83 BY 1.24 AT ROW 1.57 COL 52.67
          BGCOLOR 3 FGCOLOR 7 FONT 6
...
/*********************************************************************/  
  ...
  ASSIGN lv_monthend  =    inmot_asdat  
         ...
         fi_trndat    =    inmot_trndat 
         fi_trndatto  =    lv_monthend. 

  ASSIGN
      fi_poltyp    = "1"
      //fi_asdate    = lv_monthend
      //fi_trndat    = 09/01/2019
      //fi_trndatto  = lv_monthend
      fi_agentfr   = "0"
      fi_agentto   = "Z"
      fi_releas    = "Y"
      ...
      fi_branchto  = "Z" 
      ra_reptyp    = 2 
      fi_output    = "D:\temp\uprexp\Motor\UPMOT" + STRING(YEAR(lv_monthend),"9999") 
...
IF AVAIL  buwm100  THEN DO:
   ...
   IF MONTH(buwm100.expdat) <> MONTH(fi_asdate) AND YEAR(buwm100.expdat) <> YEAR(fi_asdate) THEN NEXT.
   
   ELSE IF MONTH(buwm100.trndat) = MONTH(fi_asdate) AND
...
ELSE DO:        /*---For not Cancel---*/
 ...
 IF MONTH(uwm100.expdat) <> MONTH(fi_asdate) AND YEAR(uwm100.expdat) <> YEAR(fi_asdate) THEN NEXT.
 IF  nv_expdat <> ? AND nv_expdat >= 0 AND nv_expdat <= 3650 THEN DO: 
        ...
        IF uwm100.tranty = "C" AND uwm100.polsta = "IF" THEN DO:
           ...
           IF AVAIL bffuwm100 THEN
              ASSIGN nvd_comdat = bffuwm100.comdat
                     nvd_expdat = bffuwm100.expdat
                     nvd_enddat = uwm100.enddat.  //uwm100.comdat.  
           ELSE ASSIGN nvd_comdat = uwm100.comdat
                       nvd_expdat = uwm100.expdat
                       nvd_enddat = uwm100.enddat.
        END.
        ...
        */
        IF nv_datecal >= nvc_expdat  /*nv_expdat*/ THEN nv_earn = n_prmcom.
        ELSE nv_earn = n_prmcom.    //ROUND((n_prmcom / nvc_expdat  /*nv_expdat*/ ) * nv_datecal,2).
        nv_uearn = ROUND(n_prmcom - nv_earn,2).   /*-- Earn , Uearn พรบ.---*/
        
        IF nv_datecal >= nvc_expdat  /*nv_expdat*/ THEN nv_earn1 = n_sumprm.
        ELSE nv_earn1 = n_sumprm.    // ROUND((n_sumprm / nvc_expdat  /*nv_expdat*/ ) * nv_datecal,2).
        nv_uearn1 = ROUND(n_sumprm - nv_earn1,2).  /*-- Earn , Uearn รถยนต์---*/
...
FOR EACH uwm100 NO-LOCK USE-INDEX uwm10092 WHERE 
    (uwm100.expdat  <=  fi_asdate AND 
    (MONTH(uwm100.expdat) = MONTH(fi_asdate) AND YEAR(uwm100.expdat) = YEAR(fi_asdate))) AND
    uwm100.agent   >=  fi_agentfr     AND
    uwm100.agent   <=  fi_agentto     AND
    uwm100.trndat  >=  fi_trndat      AND
    uwm100.trndat  <=  fi_trndatto    AND
...
FOR EACH uwm100 NO-LOCK USE-INDEX uwm10008 WHERE 
    ...
    IF uwm100.expdat <> ? THEN DO: 
       ...
       END.
       /*-------A64-0380--------------*/
       /*IF uwm100.expdat  <     fi_asdate THEN NEXT loopmain.-------------A64-0380*/
    END.
    ELSE DO:
       IF uwm100.poltyp = "C90" THEN DO:

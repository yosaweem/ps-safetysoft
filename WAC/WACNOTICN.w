...
/* CREATE BY :  Ranu I.  Assign : A58-0192  Date 19/06/2015   */
/*               Dupilcate program wacr04.w 
Wac
        -Wacnoticn.w (สำหรับสาขา NZI)
WPrl
        -Wacnoticn.prl
Whp
        -WhpBran.w
...
- Branch from - To  กำหนดให้เป็น 90-98
...
DEF  VAR report-library AS CHAR INIT "Wac/wprl/wacnoticn.prl". 
DEF  VAR report-name  AS CHAR INIT "Notic NZI". 
...
Def Var n_name     As Char Format "x(150)". /*acno name*/  /*ADD Saharat S. A63-0076 */
...
DEF VAR nv_type AS CHAR INIT "x(2)".
...
DEFINE VARIABLE fiBranch AS CHARACTER FORMAT "X(2)":U INITIAL ? 
     VIEW-AS FILL-IN 
     SIZE 6.5 BY 1
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 62.17 BY 3.76
...
DEFINE FRAME frSt
     ...
     cbAsDat AT ROW 11.1 COL 22 COLON-ALIGNED NO-LABEL
     ...
     fiPosition AT ROW 10.52 COL 82 COLON-ALIGNED NO-LABEL
     fiContractName AT ROW 13.57 COL 83.83 COLON-ALIGNED NO-LABEL
     fiExt AT ROW 14.81 COL 87.5 COLON-ALIGNED NO-LABEL
     fidoc AT ROW 5.81 COL 75.17 COLON-ALIGNED NO-LABEL
     buAcno AT ROW 8.1 COL 25
     buAcno2 AT ROW 9.29 COL 25
     brletterno AT ROW 12.81 COL 4.17
     fiBranch AT ROW 4.05 COL 11.5 COLON-ALIGNED NO-LABEL
     fiBranch2 AT ROW 5.24 COL 11.5 COLON-ALIGNED NO-LABEL
     fibdes AT ROW 4.05 COL 22.5 COLON-ALIGNED NO-LABEL
     fibdes2 AT ROW 5.24 COL 22.5 COLON-ALIGNED NO-LABEL
     ...
     fiDetail AT ROW 11.14 COL 41.67 COLON-ALIGNED NO-LABEL
     "ชื่อผู้มีอำนาจ":20 VIEW-AS TEXT
          SIZE 11.5 BY 1 AT ROW 9.19 COL 72.33
          BGCOLOR 32 FGCOLOR 2 FONT 6
     " As Of Date":20 VIEW-AS TEXT
          SIZE 17 BY 1 AT ROW 11.19 COL 5
          BGCOLOR 18 FGCOLOR 7 FONT 2
     "/" VIEW-AS TEXT
     ...
     "ตำแหน่ง" VIEW-AS TEXT
          SIZE 7.5 BY 1 AT ROW 10.52 COL 72.67
          BGCOLOR 32 FGCOLOR 2 FONT 6
     " Branch" VIEW-AS TEXT
     ...
     " Contract Name :":30 VIEW-AS TEXT
          SIZE 16.83 BY 1.05 AT ROW 13.57 COL 69
          BGCOLOR 32 FGCOLOR 2 FONT 6
     "From" VIEW-AS TEXT
          SIZE 5.5 BY 1.05 AT ROW 8 COL 5.67
          BGCOLOR 32 FGCOLOR 2 FONT 6
     "จดหมายสอบถามเบี้ยประกันภัย NZI ( Process ST A4 Daily)" VIEW-AS TEXT
     ...
     " Extension Number :":30 VIEW-AS TEXT
          SIZE 19.67 BY 1.05 AT ROW 14.81 COL 69
          BGCOLOR 32 FGCOLOR 2 FONT 6
     "To" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Notice Agent for NZI ( Process Daily)"
         HEIGHT             = 25.24
         WIDTH              = 133.33
         MAX-HEIGHT         = 25.24
         MAX-WIDTH          = 133.33
...
ASSIGN 
       fiBranch:READ-ONLY IN FRAME frSt        = TRUE.

ASSIGN 
       fiBranch2:READ-ONLY IN FRAME frSt        = TRUE.
...
ON END-ERROR OF C-Win /* Notice Agent for NZI ( Process Daily) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Notice Agent for NZI ( Process Daily) */
DO:
...
DO:
   ...
   END.
    ...
    FIND LAST AcProc_fil USE-INDEX by_type_asdat  
       WHERE (AcProc_fil.type = "01" OR AcProc_fil.type = "05") AND 
              AcProc_fil.asdat = n_asdat NO-LOCK NO-ERROR .
  ...
  IF toReprint = FALSE THEN DO:  /*----- Print New Record -----*/
      ...
      IF ( n_frac > n_toac) AND n_frac = "" AND n_toac = "" THEN DO:
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "wacnoticn".
  gv_prog  = "Notic Process ST Daily (NZI)".
  ...
  RECT-287:MOVE-TO-TOP().
    ...
    DO WITH FRAME frST:
        ASSIGN   
            ...
            n_asdat    =  DATE( INPUT cbAsDat).
            RUN pdProcbdes.
        DISPLAY  fibranch fibranch2 fiacno1 fiacno2 fibdes fibdes2  cbAsdat raY raZ.
        ...
        ASSIGN
            ...
            fiPosition  = "ฝ่ายบัญชีและการเงิน". */
            fiPowerName = nv_a4a_60  
            fiPosition  = nv_a4a_61.
            /* Benjaporn J. A59-0148 [26/04/2016]*/
...
DO:
     ...
     FOR EACH docno_fil USE-INDEX docno01 WHERE LENGTH(docno_fil.branch) = 8 AND docno_fil.poltyp = "90" NO-LOCK BREAK BY docno_fil.startno DESC.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fiacno1 fiacno2 cbAsDat raY raZ toReprint fiDoc2 fiDate fiPowerName 
          fiPosition fiContractName fiExt fidoc fiBranch fiBranch2 fibdes 
          fibdes2 fiMonth fiYear finame1 finame2 fiDetail 
      WITH FRAME frSt IN WINDOW C-Win.
  ENABLE fiacno1 fiacno2 cbAsDat raY raZ toReprint fiDoc2 fiDate fiPowerName 
         fiPosition fiContractName fiExt buAcno buAcno2 brletterno fiBranch 
         fiBranch2 fibdes fibdes2 fiMonth fiYear finame1 finame2 fiDetail 
         RECT-104 RECT-106 RECT-108 RECT-110 RECT-287 RECT-83 RECT-84 RECT-86 
         RECT-88 RECT-303 RECT-304 
      WITH FRAME frSt IN WINDOW C-Win.
...
d2 = DATE(d-mon,d-day,YEAR(TODAY) - 1).
 ...
 /* vAcProc_fil = "" .*/
  ...
  FOR EACH AcProc_fil USE-INDEX by_type_asdat  WHERE (acproc_fil.type = "01" OR acproc_fil.type = "05" )
           AND SUBSTRING(acProc_fil.enttim,10,3) <>  "NO" NO-LOCK BY asdat DESC  :
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pdCon_sicfn C-Win 
PROCEDURE pdCon_sicfn :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pdProcbdes C-Win 
PROCEDURE pdProcbdes :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DISP fibdes fibdes2 WITH FRAM frst.
...
ASSIGN
   ...
   /*-- for each เอาเลขที่จดหมายปัจจุบัน--*/
    FOR EACH docno_fil USE-INDEX docno01 WHERE LENGTH(docno_fil.branch) = 8 AND 
                                 poltyp = "90" NO-LOCK BREAK BY docno_fil.startno DESC.
        IF SUBSTR(STRING(docno_fil.startno),1,4) = nv_use THEN DO:
...
main_loop:
        FOR EACH buffagtprm WHERE buffagtprm.asdat = n_asdat AND       /* หาจำนวน Acno ทั้งหมดในช่วง*/
                buffagtprm.polbran >= n_branch AND buffagtprm.polbran <= n_branch2 AND
            ...
            IF FIRST-OF(buffAgtprm.acno) THEN DO:
                ...
                IF n_branch <> n_branch2  THEN ASSIGN nv_bran = "90". ELSE nv_bran = buffagtprm.polbran.
                   ...
                   IF LAST-OF(agtprm_fil.acno) THEN DO:                   /*create เลข running ลง docno_fil*/
                      ...
                      IF NOT AVAIL docno_fil THEN DO:
                          ...
                          ASSIGN
                              ...
                              docno_fil.poltyp  = nv_bran /*agtprm_fil.polbran*/
...
DO:
           ASSIGN 
                n_chk =0
                report-library =  "Wac/wprl/wacnoticn.prl"
                report-name    =  "Notic NZI".

            ASSIGN 
               /* nv_pic = "WIMAGE\Wasana_NZI.jpg".    Benjaporn J. A59-0148 [26/04/2016] */
                nv_pic = nv_a4a_59  /* Benjaporn J. A59-0148 [26/04/2016] */

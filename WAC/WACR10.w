...
------------------------------------------------------------------------- 
 Modify By : TANTAWAN C.   13/11/2007   [A500178]
             ...
             ขยาย format fiacno1, fiacno2 จาก "X(7)" เป็น  Char "X(10)"
-------------------------------------------------------------------------
 Modify By A50-0218  Sayamol  ปรับ Process ให้ตรงกับ Report ที่ต้องการ
 ...
 Assign No: A52-0241 รายละเอียดดังนี้
            -  เช็คข้อมูลค่า req no . เนื่องจากข้อมูลไม่ออก
            -  เก็บค่า Total calim , share(%)
...
DEF VAR n_acc     LIKE acm001.acno FORMAT "X(10)".  /*--- A500178 --- เพิ่ม FORMAT "X(10)" ---*/
...
DEFINE FRAME frTranDate
     ...
     "                      TO :":30 VIEW-AS TEXT
          SIZE 19.5 BY 1 TOOLTIP "ถึง" AT ROW 4.29 COL 12.17
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "   TRANSACTION  DATE":40 VIEW-AS TEXT
          SIZE 88.83 BY 1 TOOLTIP "วันที่ทำกรมธรรม์" AT ROW 1.19 COL 7.67
          BGCOLOR 1 FGCOLOR 7 FONT 6
     "                  FORM :":30 VIEW-AS TEXT
          SIZE 20 BY 1 TOOLTIP "ตั้งแต่" AT ROW 2.95 COL 12.17
          BGCOLOR 8 FGCOLOR 1 FONT 6
...
DEFINE FRAME frST
     ...
     "         AS OF DATE :":30 VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "TO :":20 VIEW-AS TEXT
          SIZE 5 BY .95 TOOLTIP "รหัสตัวแทนถึง" AT ROW 4.62 COL 27.67
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "FORM :":40 VIEW-AS TEXT
          SIZE 7.83 BY .95 TOOLTIP "รหัสตัวแทนเริ่มตั้งแต่" AT ROW 3.38 COL 25
          BGCOLOR 8 FGCOLOR 1 FONT 6
     "  PRODUCER  CODE":40 VIEW-AS TEXT
          SIZE 111.5 BY 1 TOOLTIP "รหัสตัวแทน" AT ROW 1.81 COL 8
          BGCOLOR 1 FGCOLOR 7 FONT 6
     "   PROCESS DATE :":30 VIEW-AS TEXT
          SIZE 19.5 BY 1 TOOLTIP "วันที่ออกรายงาน" AT ROW 9.14 COL 12.5
          BGCOLOR 8 FGCOLOR 1 FONT 6
...
/*    END.                                                                         */
   ...
   CASE CHOICE:
       WHEN TRUE THEN DO:
            ...
            RUN pdChkProcess01.  /* ตรวจสอบว่า เคยมีการ process ใน type  06 ไหม  ถ้าซ้ำ  จะไม่ process ให้ */
...
/*      SUBSTRING(acProc_fil.enttim,10,3)  = "YES") NO-ERROR.               */
    ...
    IF AVAIL acProc_fil THEN DO:
        n_chkprocess = NO.  /* พบใน 06 แล้ว process complete = "YES" */
    END.
    ELSE DO:
        n_chkprocess = YES.  /* ไม่พบ ใน  06  สามารถ process ได้*/
    END.
   ...
   /* check company is reinsurance only */
    ...
    ELSE ASSIGN
      ...
      /*-------------*/
       ...
       /*-- lukkana M. A52-0241 27/10/2009 หา docno และ total claim--*/

       FIND FIRST country_fil  WHERE 
                country_fil.ctydes  = nv_req  NO-ERROR. /* หา Req-no Lukkana M. A52-0241 28/12/2009*/ 
       ...
       FIND FIRST clm300  USE-INDEX  CLM30001    WHERE
                  ...
                  CLM300.Csftq   =  "F"          AND
                  CLM300.rico    =  acm001.acno  NO-ERROR.    /*หาค่า % share */
    ...
    FIND agtprm_fil where recid(agtprm_fil) = n_recid NO-ERROR .
       IF AVAIL agtprm_fil  THEN DO:
          ASSIGN
             ...
             agtprm_fil.comm_comp = nt_pad     /*--Lukkana M. A52-0241 27/10/2009 เก็บค่า total claim--*/
             agtprm_fil.damt     =  nv_vat1    /* Net VAT ที่มีการหัก7% Lukkana M. A52-0241 29/10/2009*/
...
/*              agtprm_fil.duedat  = IF LOOKUP(n_trntyp,nv_YZCB) = 0 THEN n_duedat ELSE acm001.trndat  /*A46-0142  acm001.trndat */ */
             ...
             /*agtprm_fil.credit   =  n_risi_p     /* percent */  ย้ายไปเก็บในฟิลด์ wcr แทน Lukkana M. A52-0241 04/12/09*/
             agtprm_fil.wcr      =  n_risi_p     /* % share ย้ายจากฟิลด์ credit มาเป็นฟิลด์นี้แทน lukkana M. A52-0241 04/12/09 */

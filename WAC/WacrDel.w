...
/* CREATE BY :  Kanchana C.        A46-0015  */
        ทำการลบข้อมูลตาม asdat ที่เลือก
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 3.91
...
DEFINE FRAME frST
     ...
     fiCount AT ROW 7.05 COL 64 COLON-ALIGNED NO-LABEL
     brAcproc_fil AT ROW 8.29 COL 4
     ...
     fiProcess AT ROW 13 COL 16
     " Process Date":30 VIEW-AS TEXT
          SIZE 12 BY 1 TOOLTIP "วันที่ออกรายงาน" AT ROW 7 COL 39
          BGCOLOR 1 FGCOLOR 7 
     ...
     "To":20 VIEW-AS TEXT
          ...
          BGCOLOR 8 
     "                               DELETE PROCESS PREMIUM STATEMENT (A4)":120 VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "wacrDel - DELETE PROCESS ST (A4)"
         HEIGHT             = 24.05
         WIDTH              = 133.33
         MAX-HEIGHT         = 24.05
         MAX-WIDTH          = 133.33
...
"acproc_fil.type" "Type" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
ON END-ERROR OF C-Win /* wacrDel - DELETE PROCESS ST (A4) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* wacrDel - DELETE PROCESS ST (A4) */
DO:
   ...
   END.
    ...
    CASE CHOICE:
        WHEN TRUE THEN DO:
            ...
            MESSAGE "ตัวแทน           : "  n_frac " ถึง " n_toac SKIP (1)
                    "Asdate            : " STRING(n_asdat,"99/99/9999")  SKIP (1)
                    "ลบข้อมูลทั้งสิ้น  " fiCount   " รายการ"  SKIP (1)
                    "เวลา  " vFirstTime "  -  " vLastTime " น."  VIEW-AS ALERT-BOX INFORMATION.
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "wacrDel".
  gv_prog  = "DELETE PROCESS PREMIUM ST (A4)".
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-94 RECT11 RECT-88 buAcno1 fiacno1 fiacno2 buAcno2 fiCount 
         brAcproc_fil finame1 finame2 fiProcessDate fiAsdat fiProcess 
      WITH FRAME frST IN WINDOW C-Win.
...
DEF VAR vCountOld AS INT.
    ...
    FOR  EACH  agtprm_fil USE-INDEX by_acno WHERE
               ...
               agtprm_fil.type   = nv_type.
               
        ACCUM  1 (count).
        
           DISP  "DELETE : " + agtprm_fil.type + " " + STRING(agtprm_fil.asdat) + " " + 

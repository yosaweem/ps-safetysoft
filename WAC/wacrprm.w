...
DEF BUFFER bfagtprm_fil FOR agtprm_fil.
DEF VAR nv_baloc LIKE acm001.baloc.
...
DEFINE FRAME frST
     ...
     "Ageing = 31/01/YYYY, Statement = Process Date" VIEW-AS TEXT
          SIZE 44 BY 1 TOOLTIP "Ageing = วันที่วันสุดท้ายของเดือน/ Statement = วันที่ ณ วันที่ Process data" AT ROW 12.24 COL 34
          BGCOLOR 19 FGCOLOR 1 
     "Branch From":30 VIEW-AS TEXT
          SIZE 12 BY .95 TOOLTIP "ตั้งแต่สาขา" AT ROW 2.81 COL 4
          BGCOLOR 8 FGCOLOR 0 
     "To":20 VIEW-AS TEXT
          SIZE 12 BY .95 TOOLTIP "รหัสตัวแทนถึง" AT ROW 8.52 COL 4
          BGCOLOR 8 FGCOLOR 0 
     ...
     "Pol. Type From":50 VIEW-AS TEXT
          SIZE 12 BY .95 TOOLTIP "ตั้งแต่สาขา" AT ROW 5.19 COL 4
          BGCOLOR 8 FGCOLOR 0 
...
DEFINE FRAME frST
     " Process Date":30 VIEW-AS TEXT
          SIZE 16 BY 1 TOOLTIP "วันที่ออกรายงาน" AT ROW 10.91 COL 34.5
          BGCOLOR 80 FGCOLOR 133 FONT 4
     "To":10 VIEW-AS TEXT
          SIZE 12 BY .95 TOOLTIP "ถึงสาขา" AT ROW 3.86 COL 4
          BGCOLOR 8 FGCOLOR 0 
     ...
     "Producer From":100 VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 0 
     "                                              Ageing Count Day for OIC (New)":210 VIEW-AS TEXT
          SIZE 107.5 BY .95 AT ROW 1 COL 13
          BGCOLOR 80 FGCOLOR 133 FONT 6
     "To":10 VIEW-AS TEXT
          SIZE 12 BY .95 TOOLTIP "ถึงสาขา" AT ROW 6.24 COL 4
          BGCOLOR 8 FGCOLOR 0 
...
DEFINE FRAME frOutput
     ...
     " Output To":40 VIEW-AS TEXT
          SIZE 48.67 BY .95 TOOLTIP "การแสดงผล" AT ROW 1 COL 1
          BGCOLOR 80 FGCOLOR 133 FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Wacrprm - Ageing Count Day for OIC (Negative)"
         HEIGHT             = 23.62
         WIDTH              = 131.67
         MAX-HEIGHT         = 31.33
         MAX-WIDTH          = 170.67
...
ON END-ERROR OF C-Win /* Wacrprm - Ageing Count Day for OIC (Negative) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Wacrprm - Ageing Count Day for OIC (Negative) */
DO:
...
/*         FRAME frST toprnIns  toPrnIns */
   ...
   END.
    ...
    IF nv_RptName = "Agent" THEN DO:  
        ...
        IF  n_toac = "" THEN  n_toac = "ZZZZZZZZZZ".  /*-- A500178 เพิ่ม format เป็น X(10)"--*/
    END.
    ELSE DO:
   ...
   IF nv_asdatAging = ? THEN DO:
    ...
    ELSE DO:
        MESSAGE "สาขา                      : "  n_Branch " ถึง " n_Branch2 SKIP(1)
                ...
                "Tran. date             : " n_trndatfr " ถึง " n_trndatto SKIP(1)
...
/* Dupplicate Program from WACRLOIC.W                  */
*************************/
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "Wacrprm".
  gv_prog  = "Ageing Count Day for OIC (Negative)".
...
/*    RUN Wut\Wutwicen.p (C-Win:HANDLE).*/
  ...
  DO WITH FRAME frTranDate:
       ASSIGN  
         rsOutput:Radio-Buttons = "File, 1" /*"หน้าจอ, 1,เครื่องพิมพ์, 2, File, 3" */
         rsOutput = 1.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE raAllbal RECT-303 RECT-305 fiTrndatFr fiTrndatTo 
      WITH FRAME frTrndat IN WINDOW C-Win.
...
FOR EACH wtGrpSum:  DELETE wtGrpSum.  END.
    ...
    RUN pdInitDataG.   /* clear ค่า GRAND TOTAL ทั้ง file detail, summary */

    ASSIGN
        nv_reccnt = 0
        nv_next   = 1
        n_OutputFile = n_OutputFile + STRING(nv_next)
        n_outputFilecom = n_OutputFile + STRING(nv_next) + "COM".
/********************** Page Header *********************/
...
/*--- DISPLAY DETAIL  ---*/
    IF (nv_DetSum = "Detail")  OR (nv_DetSum = "All") THEN DO:
        ...
        OUTPUT TO VALUE (n_OutputFile + "com" ) NO-ECHO.
            RUN pdTitleDet.    /*3 row */ /*RUN pdPageHead.*/
            RUN pdPageHeadDet. /*1*/
        OUTPUT CLOSE.

         OUTPUT TO VALUE (n_OutputFileSum + "COM") NO-ECHO.   /* COMM */
            RUN pdTitleDet.    /*3 row */ /*RUN pdPageHead.*/
            RUN pdPageHeadDet. /*1*/
        OUTPUT CLOSE.
...
/*** DISPLAY SUMMARY FILE & SUMMARY GRAND TOTAL***/
    IF (nv_DetSum = "Summary")  OR (nv_DetSum = "All") THEN DO:
        ...
        OUTPUT TO VALUE (n_OutputFileSum + "Com") NO-ECHO.  /* SUMMARY COMM */
            RUN pdTitleSum.
            RUN pdPageHeadSum.
        OUTPUT CLOSE.
        ...
        OUTPUT TO VALUE (n_OutputFileSum + "OS" + "com") NO-ECHO.  /* SUMMARY COMM*/
            RUN pdTitleSumOS.
            EXPORT DELIMITER ";"
                nv_RptName
                "Premium "
...
END.
    ...
    NO-LOCK BREAK  BY agtprm_fil.acno 
        ...
        IF agtprm_fil.bal >= 0 THEN NEXT loop_ac.
        
        n_asdat = IF nv_asdatAging = ? THEN n_asdat ELSE nv_asdatAging.

        ASSIGN nv_baloc    = 0.

        FOR EACH bfagtprm_fil NO-LOCK USE-INDEX by_acno WHERE 
                                            bfagtprm_fil.asdat  = agtprm_fil.asdat
                                        AND bfagtprm_fil.acno = agtprm_fil.acno
                                        AND bfagtprm_fil.poltyp = agtprm_fil.poltyp
                                        AND bfagtprm_fil.policy = agtprm_fil.policy
                                        AND bfagtprm_fil.docno <> agtprm_fil.docno.
    
          nv_baloc = nv_baloc + bfagtprm_fil.bal.
    
          IF SUBSTR(bfagtprm_fil.trntyp,1,1) = "M" AND nv_baloc <> 0 THEN DO:
            IF bfagtprm_fil.bal = bfagtprm_fil.comm THEN nv_baloc = 0.
          END.
    ...
    /* Begin */
        ...
        IF nv_baloc = 0 THEN DO:
         ...
         DISP agtprm_fil.acno FORMAT "X(10)"
          ...
          RUN pdDeptGrp.  
        /****** คำนวน รายละเอียดค่า และ display ใน File Detail   ใน dot i*****/
            {wac\wacrpr01.i}
        END.
        ...
        /********************** Group Footer *********************/
          IF LAST-OF(agtprm_fil.acno)  THEN DO:  /**/
            ...
            ASSIGN
                nv_RptTyp1 = ""
                nv_RptTyp2 = ""
                nv_RptTyp1 = agtprm_fil.acno  /* เปลี่ยนไปตาม ชนิดของ report */
                nv_RptTyp2 = nv_acname.

            /*** คำนวนผลรวม group, export ข้อมูลลง SUMMARY FILE และ  คำนวน SUMMARY GRAND TOTAL***/
                {wac\wacrpr02.i}
    ...
    IF (nv_DetSum = "Summary")  OR (nv_DetSum = "All") THEN DO:
        RUN pdAcnoGp.  /* แยก group code ออกมาอีก 1 file */
    END.
...
PROCEDURE pdAcnoGp :
/*------------------------------------------------------------------------------
  Purpose:     แยกไฟล์ summary  BREAK BY group code
  Parameters:  <none>
...
DEF VAR vgpname AS CHAR.
    ...
    OUTPUT TO VALUE (n_OutputFile + "Gp" + "com") NO-ECHO.  /* SUMMARY COMM */
        RUN pdTitleSum.
        RUN pdPageHeadSum.
        ...
        FOR EACH wtGrpSum USE-INDEX wtGrpSum2
            ...
            IF FIRST-OF(wtGTgpcode) THEN DO:
                ...
                IF AVAIL xmm600 THEN vgpname = xmm600.name.  /* ชื่อ พนักงาน การเงิน */
                                 ELSE vgpname = "".
            ...
            END.
                ...
                EXPORT DELIMITER ";"  wtGTrptname + " Premium"          wtGTTprem   wtGTprem[1 FOR 10].  
                EXPORT DELIMITER ";"  wtGTrptname + " Commission"       wtGTTcomm   wtGTcomm[1 FOR 10].  
                EXPORT DELIMITER ";"  wtGTrptname + " Bal. O/S"         wtGTTbal    wtGTbal[1 FOR 10].   
                EXPORT DELIMITER ";" wtGTrptname + " Cheque Returned"   wtGTTretc   wtGTretc[1 FOR 10].  
                EXPORT DELIMITER ";"  wtGTrptname + " Suspense"         wtGTTsusp   wtGTsusp[1 FOR 10].  
                EXPORT DELIMITER ";"  wtGTrptname + " Net. A/R"         wtGTTnetar  wtGTnetar[1 FOR 10]. 
                EXPORT DELIMITER ";"  wtGTrptname + " Net. OTHER"       wtGTTnetoth wtGTnetoth[1 FOR 10].

                EXPORT DELIMITER ";" "Summary of : " + wtGTrptname + " " + wtGTacno
                    "Within"         + STRING(wtGTbal[1] ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                    "ไม่เกิน 30 วัน" + STRING(wtGTbal[2] ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                    "31 - 60 วัน"    + STRING(wtGTbal[3] ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                    "61 - 90 วัน"    + STRING(wtGTbal[4] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "91 - 120 วัน"   + STRING(wtGTbal[5] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "121 - 180 วัน"  + STRING(wtGTbal[6] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "181 - 365 วัน"  + STRING(wtGTbal[7] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "366 วันขึ้นไป"  + STRING(wtGTbal[8] ,">>,>>>,>>>,>>9.99-") .
        ...
        EXPORT DELIMITER ";"  "Grand Premium"           nv_GTTprem      nv_GTprem[1 FOR 10]. 
        EXPORT DELIMITER ";"  "Grand Commission"        nv_GTTcomm      nv_GTcomm[1 FOR 10]. 
        EXPORT DELIMITER ";"  "Grand Bal. O/S"          nv_GTTbal       nv_GTbal[1 FOR 10].   
        EXPORT DELIMITER ";"  "Grand Cheque Returned"   nv_GTTretc      nv_GTretc[1 FOR 10].  
        EXPORT DELIMITER ";"  "Grand Suspense"          nv_GTTsusp      nv_GTsusp[1 FOR 10].  
        EXPORT DELIMITER ";"  "Grand Net. A/R"          nv_GTTnetar     nv_GTnetar[1 FOR 10]. 
        EXPORT DELIMITER ";"  "Grand Net. OTHER"        nv_GTTnetoth    nv_GTnetoth[1 FOR 10].
        EXPORT DELIMITER ";"  "".
        EXPORT DELIMITER ";" "G R A N D  T O T A L"
...
/*** DISPLAY SUMMARY FILE & SUMMARY GRAND TOTAL***/
    ...
    IF (nv_DetSum = "Summary")  OR (nv_DetSum = "All") THEN DO:
        ...
        OUTPUT TO VALUE (n_OutputFileSum + "OSIC" + "com") NO-ECHO.  /* SUMMARY COMM */
            RUN pdTitleSumOS.
            EXPORT DELIMITER ";"
                nv_RptName
                "Name"
    ...
    IF to_type = NO THEN RUN pdAgentDet_po.  /*ระบุ Pol Type*/
...
------------------------------------------------------------------------------*/
    ...
    NO-LOCK BREAK  BY agtprm_fil.agent 
        ...
        FOR EACH bfagtprm_fil NO-LOCK USE-INDEX by_agent WHERE 
                                           bfagtprm_fil.asdat  = agtprm_fil.asdat
                                        AND bfagtprm_fil.agent = agtprm_fil.agent
                                        AND bfagtprm_fil.poltyp = agtprm_fil.poltyp
                                        AND bfagtprm_fil.policy = agtprm_fil.policy
                                        AND bfagtprm_fil.docno <> agtprm_fil.docno.
        ...
        /********************** Group Footer *********************/
          IF LAST-OF(agtprm_fil.agent)  THEN DO:  /**/
            ASSIGN
                nv_RptTyp1 = ""
                nv_RptTyp2 = ""
                nv_RptTyp1 = agtprm_fil.agent  /* เปลี่ยนไปตาม ชนิดของ report */
                nv_RptTyp2 = nv_acname.
                ...
                /*---A53-0159---*/
                 ...
                 BREAK BY wtDetAgent 
                     ...
                     EXPORT DELIMITER ";"
                       ...
                       wtDetnetar
                       wtDetnetoth
                       wtDetbalDet[1 FOR 10]
                       .
                    OUTPUT CLOSE.

                    OUTPUT TO VALUE (STRING(n_OutputFile + "com" ) ) APPEND NO-ECHO.
                     EXPORT DELIMITER ";"
                       "ToTal of " + wtDetagent + " - " + wtDetpolgrp + " - " + wtDetinsref
                       ""
                       ""
...
nv_poltyp = "".
 ...
 IF rstype = 1 THEN DO:
    FOR EACH xmm031 NO-LOCK WHERE xmm031.dept = "M" OR 
     ...
     BREAK BY xmm031.poltyp:
        IF nv_poltyp = "" THEN 
           ASSIGN nv_poltyp = xmm031.poltyp.
        ELSE 
           ASSIGN nv_poltyp = nv_poltyp + "," + xmm031.poltyp .
    END.
 END.
 ELSE IF rstype = 2 THEN DO:
    FOR EACH xmm031 NO-LOCK WHERE xmm031.dept <> "M" OR 
...
ASSIGN nv_GTTprem     = 0
    ...
    OUTPUT TO VALUE (n_OutputFile + "sumIC" + "com") NO-ECHO.  /* SUMMARY */
        RUN pdTitleSum.
        RUN pdPageHeadSum.
        OUTPUT CLOSE.
        ...
        FOR EACH wtoicSum BREAK BY wtICAgent 
            ...
            IF FIRST-OF(wtoicSum.wtICAgent) THEN DO:
                ...
                IF AVAIL xmm600 THEN agpname = xmm600.name.  /* ชื่อ พนักงาน การเงิน */
                ELSE agpname = "".
                ...
                OUTPUT TO VALUE (n_OutputFile + "sumIC" + "com") APPEND NO-ECHO.  /* SUMMARY */    
                    EXPORT DELIMITER ";" "".
                    EXPORT DELIMITER ";" "AGENT CODE : " + wtICAgent + " - " +  agpname.
                    EXPORT DELIMITER ";" FILL("-",100).
                OUTPUT CLOSE.
            ...
            OUTPUT TO VALUE (n_OutputFile + "sumIC") APPEND NO-ECHO.  /* SUMMARY */    
                ...
                EXPORT DELIMITER ";"  wtICrptname + " Premium"          wtICTprem   wtICprem[1 FOR 10].  
                EXPORT DELIMITER ";"  wtICrptname + " Commission"       wtICTcomm   wtICcomm[1 FOR 10].  
                EXPORT DELIMITER ";"  wtICrptname + " Bal. O/S"         wtICTbal    wtICbal[1 FOR 10].  
                EXPORT DELIMITER ";"  wtICrptname + " Cheque Returned"  wtICTretc   wtICretc[1 FOR 10].  
                EXPORT DELIMITER ";"  wtICrptname + " Suspense"         wtICTsusp   wtICsusp[1 FOR 10].  
                EXPORT DELIMITER ";"  wtICrptname + " Net. A/R"         wtICTnetar  wtICnetar[1 FOR 10]. 
                EXPORT DELIMITER ";"  wtICrptname + " Net. OTHER"       wtICTnetoth wtICnetoth[1 FOR 10].
                EXPORT DELIMITER ";" "Summary of : " + wtICrptname + " " + wtICAgent + " :: " + wtICpolgrp + " :: " + wtICinsref    
                        "Within"            + STRING(wtICprem[1]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "ไม่เกิน 30 วัน"    + STRING(wtICprem[2]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "31 - 60 วัน"       + STRING(wtICprem[3]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "61 - 90 วัน"       + STRING(wtICprem[4]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "91 - 120 วัน"      + STRING(wtICprem[5]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "121 - 180 วัน"     + STRING(wtICprem[6]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "181 - 365 วัน"     + STRING(wtICprem[7]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "366 วันขึ้นไป"     + STRING(wtICprem[8]  ,">>,>>>,>>>,>>9.99-").
             ...
             OUTPUT TO VALUE (n_OutputFile + "sumIC" + "com") APPEND NO-ECHO.  /* SUMMARY */    
                ...
                EXPORT DELIMITER ";"  wtICrptname + " Net. OTHER"       wtICTnetoth wtICnetoth[1 FOR 10].
                EXPORT DELIMITER ";" "Summary of : " + wtICrptname + " " + wtICAgent + " :: " + wtICpolgrp + " :: " + wtICinsref    
                        "Within"            + STRING(wtICcomm[1]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "ไม่เกิน 30 วัน"    + STRING(wtICcomm[2]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "31 - 60 วัน"       + STRING(wtICcomm[3]  ,">>,>>>,>>>,>>9.99-") + nv-1 + 
                        "61 - 90 วัน"       + STRING(wtICcomm[4]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "91 - 120 วัน"      + STRING(wtICcomm[5]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "121 - 180 วัน"     + STRING(wtICcomm[6]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "181 - 365 วัน"     + STRING(wtICcomm[7]  ,">>,>>>,>>>,>>9.99-") + nv-1 +
                        "366 วันขึ้นไป"     + STRING(wtICcomm[8]  ,">>,>>>,>>>,>>9.99-").
           ...
           OUTPUT TO VALUE (n_OutputFileSum + "OSIC" + "com") APPEND NO-ECHO.  /* SUMMARY */
               EXPORT DELIMITER ";"
               wtoicSum.wtICAgent
               wtICacname     
               wtICTprem      /*"Premium "*/
           ...
           OUTPUT CLOSE.
            ...
            nv_GTcomm[1]   =    nv_GTcomm[1]  +  wtICcomm[1]                          
            nv_GTcomm[2]   =    nv_GTcomm[2]  +  wtICcomm[2]                          
            nv_GTcomm[3]   =    nv_GTcomm[3]  +  wtICcomm[3]                          
            nv_GTcomm[4]   =    nv_GTcomm[4]  +  wtICcomm[4]                          
            nv_GTcomm[5]   =    nv_GTcomm[5]  +  wtICcomm[5]                          
            nv_GTcomm[6]   =    nv_GTcomm[6]  +  wtICcomm[6]                          
            nv_GTcomm[7]   =    nv_GTcomm[7]  +  wtICcomm[7]                          
            nv_GTcomm[8]   =    nv_GTcomm[8]  +  wtICcomm[8]                          
            nv_GTcomm[9]   =    nv_GTcomm[9]  +  wtICcomm[9] 
            nv_GTcomm[10]  =    nv_GTcomm[10] +  wtICcomm[10]
           .                                    
        ...
        EXPORT DELIMITER ";"  "".
        EXPORT DELIMITER ";" "G R A N D  T O T A L"
                "Within"         + STRING(nv_GTprem[1] ,">>,>>>,>>>,>>9.99-") + nv-1 +     
                "ไม่เกิน 30 วัน" + STRING(nv_GTprem[2] ,">>,>>>,>>>,>>9.99-") + nv-1 +     
                "31 - 60 วัน"    + STRING(nv_GTprem[3] ,">>,>>>,>>>,>>9.99-") + nv-1 +  
                "61 - 90 วัน"    + STRING(nv_GTprem[4] ,">>,>>>,>>>,>>9.99-") + nv-1 +           
                "91 - 120 วัน"   + STRING(nv_GTprem[5] ,">>,>>>,>>>,>>9.99-") + nv-1 +     
                "121 - 180 วัน"  + STRING(nv_GTprem[6] ,">>,>>>,>>>,>>9.99-") + nv-1 +     
                "181 - 365 วัน"  + STRING(nv_GTprem[7] ,">>,>>>,>>>,>>9.99-") + nv-1 +     
                "366 วันขึ้นไป"  + STRING(nv_GTprem[8] ,">>,>>>,>>>,>>9.99-").  
...
IF nv_RptName = "agent" THEN DO:
    ...
    IF NOT AVAIL wtDet THEN DO:
        ...
        ASSIGN wtDet.wtDetbaldet = 0
               ...
               wtDet.wtDETcomdet[lip]  =  nv_comdet[lip].
    ...
    ELSE DO:
        ASSIGN wtDet.wtDETprem   = wtDet.wtDETprem     +  nv_premA  
               ...
               wtDet.wtDETcomdet[lip]  =  wtDet.wtDETcomdet[lip]  +  nv_comdet[lip].
...
FIND FIRST xmm031 USE-INDEX xmm03101 WHERE xmm031.poltyp = agtprm_fil.poltyp NO-LOCK.
IF AVAIL xmm031 THEN DO:
    IF xmm031.dept = "G" OR xmm031.dept = "M" THEN DO:   /*Motor*/
       IF xmm031.dept = "G" THEN 
          nv_polgrp = "MOT".
       ELSE  
          nv_polgrp = "CMP".
    END.
...
IF AVAIL acm001 THEN DO:
   IF LENGTH(acm001.insref) = 7 THEN DO:
      IF SUBSTR((acm001.insref),2,1) = "C" THEN 
         nv_insref = "Coperate".   /*Customer Type = Corperate */
      ELSE  
         nv_insref = "Personal". 
   END.
   ELSE DO:
      IF SUBSTR((acm001.insref),3,1) = "C" THEN
         nv_insref = "Coperate".   /*Customer Type = Corperate */
      ELSE 
         nv_insref = "Personal".   /*Customer Type = Personal */
   END.
END.
ELSE DO:
...
DEF VAR vgpname AS CHAR.
    ...
    NO-LOCK:
        ...
        IF AVAIL bxmm600 THEN vgpname = bxmm600.name.  /* ชื่อ พนักงาน การเงิน */
                         ELSE vgpname = "".
        CREATE wtGrpCode.
        ASSIGN
...
/*------------------------------------------------------------------------------
  Purpose:      use in Procedure pdAcnoDet, pdBranchDet, pdLineDet :
  Parameters:  <none>
  Notes:       clear ค่า summary
------------------------------------------------------------------------------*/
     ASSIGN
     /*---  Initial Value total ใน  file summary   ---*/
        nv_tot_prem  = 0
        nv_tot_prem_comp = 0
        nv_tot_stamp = 0
        nv_tot_tax   = 0
     ...
     /*---  Initial Value array [i] ผลรวมใน file summary    "Summary of : " ---*/
        nv_Tprem  = 0
        nv_Ttax   = 0
        nv_Tvat   = 0
        nv_Tsbt   = 0
    ...
    FOR EACH  agtprm_fil USE-INDEX by_agent WHERE 
      ...
      (LOOKUP (SUBSTRING(agtprm_fil.trntyp,1,1) , nv_trntyp1) <> 0 ) 
       NO-LOCK BREAK BY SUBSTRING(agtprm_fil.policy,3,2)
        ...
        IF  agtprm_fil.bal >= 0 THEN NEXT loop_li.
        ...
        /********************** Group Footer *********************/
         IF LAST-OF(SUBSTRING(agtprm_fil.policy,3,2))  THEN DO:  /**/
            ASSIGN
                nv_RptTyp1 = ""
                nv_RptTyp2 = ""
                nv_RptTyp1 = SUBSTRING(agtprm_fil.policy,3,2)  /* เปลี่ยนไปตาม ชนิดของ report */
...
OUTPUT TO VALUE (n_OutputFile + "sumIC" + "com") NO-ECHO.  /* SUMMARY COMM*/
RUN pdTitleSum.
RUN pdPageHeadSum.
OUTPUT CLOSE.
...
FOR EACH wtoicSum BREAK BY wtICPoltyp
    ...
    IF FIRST-OF(wtoicSum.wtICPoltyp) THEN DO:
        ...
        OUTPUT TO VALUE (n_OutputFile + "sumIC" + "com") APPEND NO-ECHO.  /* SUMMARY COMM*/    
        EXPORT DELIMITER ";" "".
        EXPORT DELIMITER ";" "Line : " + wtICPoltyp + " - " +  liname.
        EXPORT DELIMITER ";" FILL("-",100).
        OUTPUT CLOSE.
    ...
    OUTPUT CLOSE.                       
        ...
        IF NOT AVAIL wtnTLi THEN DO:
            ...
            ASSIGN wtntli.ntline   = wtoicSum.wticpoltyp
                   ...
                   wtnTli.nTTbalc[1]  = wtoicSum.wtICcomm[1]
                   wtnTli.nTTbalc[2]  = wtoicSum.wtICcomm[2]
                   wtntli.nTTbalc[3]  = wtoicSum.wtICcomm[3]
                   wtnTli.nTTbalc[4]  = wtoicSum.wtICcomm[4]
                   wtnTli.nTTbalc[5]  = wtoicSum.wtICcomm[5]
                   wtnTli.nTTbalc[6]  = wtoicSum.wtICcomm[6]
                   wtntli.nTTbalc[7]  = wtoicSum.wtICcomm[7]
                   wtnTli.nTTbalc[8]  = wtoicSum.wtICcomm[8]
                   wtnTli.nTTbalc[9]  = wtoicSum.wtICcomm[9]
                   wtnTli.nTTbalc[10]  = wtoicSum.wtICcomm[10]
                   .
        ...
        ELSE DO:
            ASSIGN  wtnTli.ntprem   = wtnTli.ntprem   + wtoicSum.wtICTprem  
                    ...
                    wtnTli.nTTbalc[1]  = wtnTli.nTTbalc[1]  + wtoicSum.wtICcomm[1] 
                    wtnTli.nTTbalc[2]  = wtnTli.nTTbalc[2]  + wtoicSum.wtICcomm[2] 
                    wtntli.nTTbalc[3]  = wtntli.nTTbalc[3]  + wtoicSum.wtICcomm[3] 
                    wtnTli.nTTbalc[4]  = wtnTli.nTTbalc[4]  + wtoicSum.wtICcomm[4] 
                    wtnTli.nTTbalc[5]  = wtnTli.nTTbalc[5]  + wtoicSum.wtICcomm[5] 
                    wtnTli.nTTbalc[6]  = wtnTli.nTTbalc[6]  + wtoicSum.wtICcomm[6] 
                    wtntli.nTTbalc[7]  = wtntli.nTTbalc[7]  + wtoicSum.wtICcomm[7] 
                    wtnTli.nTTbalc[8]  = wtnTli.nTTbalc[8]  + wtoicSum.wtICcomm[8] 
                    wtnTli.nTTbalc[9]  = wtnTli.nTTbalc[9]  + wtoicSum.wtICcomm[9]
                    wtnTli.nTTbalc[10] = wtnTli.nTTbalc[10] + wtoicSum.wtICcomm[10]
                    .
        END.
         ...
         ASSIGN 
            ...
            nv_GTcomm[1]   =    nv_GTcomm[1]  +  wtnTli.nTTbalc[1]                         
            nv_GTcomm[2]   =    nv_GTcomm[2]  +  wtnTli.nTTbalc[2]                         
            nv_GTcomm[3]   =    nv_GTcomm[3]  +  wtntli.nTTbalc[3]                         
            nv_GTcomm[4]   =    nv_GTcomm[4]  +  wtnTli.nTTbalc[4]                         
            nv_GTcomm[5]   =    nv_GTcomm[5]  +  wtnTli.nTTbalc[5]                         
            nv_GTcomm[6]   =    nv_GTcomm[6]  +  wtnTli.nTTbalc[6]                         
            nv_GTcomm[7]   =    nv_GTcomm[7]  +  wtntli.nTTbalc[7]                         
            nv_GTcomm[8]   =    nv_GTcomm[8]  +  wtnTli.nTTbalc[8]                         
            nv_GTcomm[9]   =    nv_GTcomm[9]  +  wtnTli.nTTbalc[9]
            nv_GTcomm[10]  =    nv_GTcomm[10] +  wtnTli.nTTbalc[10]
            .    
...
EXPORT DELIMITER ";"
    ...
    nv_blankH[1 FOR 14]       /* ใส่ ช่องว่าง  14 ช่อง*/
...
EXPORT DELIMITER ";"
    "Branch : " + n_Branch + " To " + n_Branch2
     nv_blankH[1 FOR 14]
    "Ageing Report By " + nv_RptName + " " + nv_RptName2 + " " + "for OIC (New) - Negative Premium"
     nv_blankH[1 FOR 14]
...
EXPORT DELIMITER ";"
    "From : " + n_frac + " To " + n_toac
    nv-1  /*       nv_blankH[1 FOR 14]       /* ใส่ ช่องว่าง  14 ช่อง*/*/
    "SAFETY INSURANCE PUBLIC COMPANY LIMITED"
    nv-1
    "RUN DATE : " + STRING(TODAY,"99/99/9999").
...
EXPORT DELIMITER ";"
    "From : " + n_frac + " To " + n_toac
    nv_blankH[1 FOR 10]       /* ใส่ ช่องว่าง  14 ช่อง*/
    "SAFETY INSURANCE PUBLIC COMPANY LIMITED"
    nv_blankH[1 FOR 5]
...
EXPORT DELIMITER ";"
    "Branch : " + n_Branch + " To " + n_Branch2
     nv_blankH[1 FOR 10]
    "Ageing Report By " + nv_RptName + " " + nv_RptName2 + " " + "for OIC (New)"
     nv_blankH[1 FOR 5]
...
EXPORT DELIMITER ";"
    "From : " + n_frac + " To " + n_toac
    nv_blankH[1 FOR 2]       /* ใส่ ช่องว่าง  5 ช่อง*/
    "SAFETY INSURANCE PUBLIC COMPANY LIMITED"
    nv_blankH[1 FOR 3]
    "Run Date : " + STRING(TODAY,"99/99/9999") + "  " + "Run Time : " + STRING(TIME,"HH:MM:SS").

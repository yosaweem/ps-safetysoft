    ...
    IF  raAllbal = 2 AND
      ((towcr   = FALSE) AND
       ...
       (toodue5 = FALSE))  THEN DO:
         MESSAGE  "กรุณาระบุ Bal. Filter" VIEW-AS ALERT-BOX WARNING . 
         RETURN  NO-APPLY .
    END.    
...
Wut
    ...
    1. ต้องการให้เพิ่มช่องที่ระบุ Trntyp1
    /* ช่องแรก กำหนดวันที่เริ่มต้น  "With Credit Term, 1,Transaction Date, 2" */
    กรณีเลือก report  "With Credit Term" THEN agtprm_fil.duedat  ในการเทียบค่าเริ่มต้น  วันที่ startdate + credit term
                               "Transaction Date" THEN  agtprm_fil.trndat. เริ่มต้นที่ trandate เลย
    2. ทดแทน Aging Report ที่ใช้อยู่ในระบบ premium เดิม
        ข้อ 15.01  Aging By Group AC                  azd400.p ,  azd40001.p
        ข้อ 15.02  Aging By Group AC (Summ.)   azd403.p,  azd40301.p
        ข้อ 15.03  Aging By Line   AC   (Summ.)  azd401.p,  azd40101.p
        ข้อ 15.13  Aging By Grp. AC (Prem/Vat) azg40300.p,   azg403.p,    azg40301.p
        process type   เดิม
        n_trnty1[1]  =  "M"        n_trnty1[2]  =  "R"        n_trnty1[3]  =  "A"
        n_trnty1[4]  =  "B"        n_trnty1[5]  =  "Y"        n_trnty1[6]  =  "Z"
        มีรายละเอียด ดังนี้
        1. ใช้ข้อมูลจากการ  Process Statement A4
        ...
        3. แยกประเภท Report     รูปแบบการคำนวน ว่าจำนวนเงินจะตกอยู่ในช่วงใด 
            1. ตาม With Credit Term (จะบวก credit term เข้าไปก่อน  แล้วจะคำนวนว่ายอด over due ไปกี่เดือน) จะใช้วันที่ใน agtprm_fil.duedat มาเทยบ
            2. ตาม Transaction Date (จะคำนวนว่ายอดค้างนะ เดือนนั้น ๆ  เป็นเท่าไร)                                                    ..      agtprm_fil.trndat    มาเทียบ

            ซึ่ง Report ทั้ง 2 นี้ จะเรียกข้อมูลออกมาได้  3 แบบ  ตาม
            - By Acno (เดิม)  Aging จะดึงข้อมูล By Group code  field   xmm600.gpage
                             (ใหม่) ดีงข้อมูล By Acno <Producer Code>
                             แยกไฟล์ summary  BREAK BY group code 
            - By Branch
            - By Line
        4. output  ที่ได้  เป็น text file ที่มี ";"  คั่นอยู่  เพื่อนำไปเข้าใน  Excel  คำนวน และทำการ print ได้จาก excel
                ...
                1. ไฟล์ detail          (รุปแบบคล้ายกับใน Statement A4 to File)
                2. ไฟล์  summary  (ในท้ายไฟล์จะสรุปยอด  With In credit , Due amount, over 1-3, over 3-6, over 6-9, over 9-12, over 12)
        -  ไฟล์ข้อมูลที่ออกมานั้น เมื่อนำเข้า Excel  หากต้องการรูปแบบที่สวยงานควรใช้ Font  MS Sans Serif  
...
1. เพิ่มการหาลูกหนี้เกินปี
2. แยก excel file เกิน 65500 record  ให้ขึ้น  file ใหม่
3. เลือกว่าจะแสดง  insure name หรือไม่ก้ได้
4. แยก file  age-os   prem, comm, os bal
...
/*         /* A47-0264 */                                            */
/*         IF  raAllbal = 2  THEN DO:      /* ดึงข้อมูลบางส่วน */    */
...
DEF VAR  i AS INT.
    ...
    NO-LOCK BREAK BY agtprm_fil.acno:
     ...
     /*================== เปรียบเทียบวันที่ As Date กับ duedat & odat1-4 (over due date) ===*/
            IF /*n_day <> 0  AND*/ n_asdat <= (agtprm_fil.duedat - fuMaxDay(agtprm_fil.duedat)) THEN DO:  /* เทียบ asdate กับ  วันทีสุดท้าย  ก่อนเดือนสุดท้าย*/
                n_wcr = n_wcr + agtprm_fil.bal .                  /* with in credit  ไม่ครบกำหนดชำระ */
            END.
            IF n_asdat > (agtprm_fil.duedat - fuMaxDay(agtprm_fil.duedat)) AND n_asdat <= agtprm_fil.duedat THEN DO:   /*เทียบ asdate กับวันที่ในช่วงเดือนสุดท้าย*/
...
EXPORT DELIMITER ";"
    "From : " + n_frac + " To " + n_toac
    nv_blankH[1 FOR 5]       /* ใส่ ช่องว่าง  5 ช่อง*/
    "SAFETY INSURANCE PUBLIC COMPANY LIMITED"
    nv_blankH[1 FOR 6]
    "Run Date : " + STRING(TODAY,"99/99/9999") + "  " + "Run Time : " + STRING(TIME,"HH:MM:SS").

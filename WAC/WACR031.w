...
/************   Program   **************
Wac
        -Wacr03.w            /*PRINT PREMIUM STATEMENT (A4)*/
WPrl
...
Wut
        ...
        -WutWiCen.p
*****************************************/
/* CREATE BY :  Kanchana C.  2002  */
/* Modify : Kanchana C.  31/10/2002  
                แก้ไข  ออก text file ให้ออกเป็น text file คั่นด้วย ";"  แล้วจึงนำเข้า Excel   */

/* Modify : Kanchana C.  02/12/2002
                By Branch   ,  Sum ตาม Branch
                -Wac_sm02.prl  = "Check List ST(A4) By Branch" 
                -Wac_sm04.prl  = "Check List ST(A4) By Br to Excel"               */

/* Modify : Kanchana C.  13/01/2003   Assign No.  :   A46-0026
               ต้องการให้  เพิ่ม แสดง ข้อมูล credit limit   ใน Statement A4   summary to file */

/* Modify : Kanchana C.  21/02/2003   Assign No.  :   A46-0092
               ต้องการให้เพิ่มช่องที่ระบุ type 
               เดิมใช้ Report builder ใส่ ";"
               เปลี่ยนเป็น Export delimiter ";" */
               
/* Modify : Kanchana C.  13/01/2004   Assign No.  :   A47-0014
               ต้องการให้เพิ่ม
               field_groupcode ใน  Report  "Check List Statement to Excel by Branch" 
               groupcode    =  xmm600.gpstmt
                เพิ่ม เฉพาะ output to  file excel  เท่านั้น
                ออกเป็น 2 file
                1. ชื่อไฟล์ปกติ  (ตัด GROUP Header - Footer ออก)
                2. ชื่อไฟล์ปกติ + "H" เหมือนเดิม (เพิ่ม field_groupcode )
                */

/* Modify : Kanchana C.  03/02/2004   Assign No.  :   A47-0035
            ต้องการให้เพิ่ม
            1. report by producer : field_groupcode , total Prem, total Comm
            2. เพิ่ม report by Agent  เพิ่ม field agent,  group code
   xmm600.gpstmt = nv_agent
*/

/* Modify : Kanchana C.  13/12/2004   Assign No.  :   A47-0500
    report-name = "Summary by Agent"
    เรียก agent code from - to 
    
   xmm600.gpstmt = nv_agent BY agent BY acno
*/

/* Modify : Kanchana C.  27/12/2004   Assign No.  :   A47-9999

   toOut = ส่งกรมการประกันภัย                                   
   xmm600.gpstmt = nv_agent   BY agent
*/
...
/*------------------------------------------------------------------------- 
 Modify By : TANTAWAN C.   09/11/2007   [A500178]
             ...
             ยขยาย format fiagent  จาก "X(7)" เป็น  Char "X(10)"
                          fiagent2 จาก "X(7)" เป็น  Char "X(10)"
             เปลี่ยนมาใช้ branch ที่ฟิลด์ uwm100.branch ไม่ต้อง supstring
-------------------------------------------------------------------------*/
/* Modify By : Sayamol N.  08/09/2011 (A54-0270)
     - แก้ไขหัวคอลัมน์เป็น 1-30 days  31-60 days 61-90 days 91-365 days over 365 days 
*/    
...
*/     
/* Modify By : Eakkapong  17/072014 (A57-0256)  
     - แก้ไขโปรแกรมเรียกรายงาน collection report  ให้โปรแกรมสามารถ
     ดึงข้อมูล ชื่อผู้ติดต่อเก็บเงิน, เบอร์ติดต่อ เบอร์โทรสาร และ e-mail  ได้ 
     และเพิ่มเมนู AG/BR , FI/Dealer , LS , ALL และเพิ่ม column ในรายงาน
*/       

/* Modify By : Eakkapong  29/09/2014 (A57-0357)  
     - เพิ่มเงื่อนไขให้ดึงข้อมูล Credit Limit / Overdue
*/    
/*----------------------------------------------------------------------
     Modify By: Saharat S.  A62-0279  06/12/2019
...
DEF  VAR nv_tot_bal1         AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".
...
DEF  VAR nv_brtot_odue      AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/

DEF  VAR nv_brtot_odue1     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_brtot_odue2     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_brtot_odue3     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_brtot_odue4     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_brtot_odue5     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
...
DEF  VAR nv_agtot_odue      AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
                            
DEF  VAR nv_agtot_odue1     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_agtot_odue2     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_agtot_odue3     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_agtot_odue4     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
DEF  VAR nv_agtot_odue5     AS   DECI   FORMAT ">>,>>>,>>>,>>9.99-".  /*overdue*/
...
/* A47-0014 */
/*------ temp table for fncode in acno  ---*/
DEF  TEMP-TABLE  wtgp          /* workfile for    group code statement   รายชื่อผูดูแลแต่ละตัวแทน */
    FIELD wtgpstmt  AS CHAR FORMAT "X(10)" /*--- A500178 --- เพิ่ม format X(10)---*/
...
DEF VAR nv_ReportTyp  AS CHAR. /*by eakkapong  A57-0240 */

DEF VAR nv_credit1  AS DECI FORMAT "->>,>>>,>>>,>>9.99".
...
DEFINE BUTTON buAdd 
     LABEL "Add" 
     SIZE 8 BY .95 TOOLTIP "????? Client Type Code".
...
"Over", 2,
"All", 3
     SIZE 11.67 BY 3.48 NO-UNDO.

DEFINE VARIABLE ra_overdue AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "OverDue", 1,
"Within", 2,
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 68.33 BY 3.91
...
DEFINE FRAME frST
     ra_select AT ROW 11.91 COL 80.5 NO-LABEL
     ra_credit AT ROW 13.33 COL 80.5 NO-LABEL
     toOut AT ROW 3.1 COL 55.5
     cbRptList AT ROW 2.95 COL 19.67 COLON-ALIGNED NO-LABEL
     brAcproc_fil AT ROW 5.67 COL 3.67
     ...
     fiTyp1 AT ROW 12.14 COL 48.83 COLON-ALIGNED NO-LABEL
     fiTyp2 AT ROW 12.14 COL 53.83 COLON-ALIGNED NO-LABEL
     fiTyp3 AT ROW 12.14 COL 58.83 COLON-ALIGNED NO-LABEL
     fiTyp4 AT ROW 12.14 COL 63.83 COLON-ALIGNED NO-LABEL
     fiTyp5 AT ROW 12.14 COL 68.83 COLON-ALIGNED NO-LABEL
     ...
     fiTyp6 AT ROW 13.43 COL 48.83 COLON-ALIGNED NO-LABEL
     fiTyp7 AT ROW 13.43 COL 53.83 COLON-ALIGNED NO-LABEL
     fiTyp8 AT ROW 13.43 COL 58.83 COLON-ALIGNED NO-LABEL
     fiTyp9 AT ROW 13.43 COL 63.83 COLON-ALIGNED NO-LABEL
     fiTyp10 AT ROW 13.43 COL 68.83 COLON-ALIGNED NO-LABEL
     fiTyp11 AT ROW 14.71 COL 48.83 COLON-ALIGNED NO-LABEL
     fiTyp12 AT ROW 14.71 COL 53.83 COLON-ALIGNED NO-LABEL
     fiTyp13 AT ROW 14.71 COL 58.83 COLON-ALIGNED NO-LABEL
     fiTyp14 AT ROW 14.71 COL 63.83 COLON-ALIGNED NO-LABEL
     fiTyp15 AT ROW 14.71 COL 68.83 COLON-ALIGNED NO-LABEL
     fiAsdat AT ROW 4.24 COL 19.67 COLON-ALIGNED
     fiProcessDate AT ROW 4.24 COL 56.17 COLON-ALIGNED NO-LABEL
     fibdes AT ROW 11.52 COL 19.33 COLON-ALIGNED NO-LABEL
     fiInclude AT ROW 10.76 COL 69.5 COLON-ALIGNED
     ...
     fiProcess AT ROW 15.71 COL 51.83
     raReportTyp AT ROW 2.81 COL 68.67 NO-LABEL
     seCliCod AT ROW 5.19 COL 78.5 NO-LABEL
     fiCcode1 AT ROW 5.19 COL 90 COLON-ALIGNED NO-LABEL
     buClicod AT ROW 5.19 COL 97
     buAdd AT ROW 7 COL 92
     buDel AT ROW 8.29 COL 92
     ra_overdue AT ROW 13.33 COL 94.33 NO-LABEL
     ...
     " Type Of Reports":50 VIEW-AS TEXT
          SIZE 17.5 BY .95 TOOLTIP "ประเภทรายงาน" AT ROW 2.95 COL 3.17
          BGCOLOR 1 FGCOLOR 7 FONT 6
     " Process Date":30 VIEW-AS TEXT
          SIZE 17.5 BY 1 TOOLTIP "วันที่ออกรายงาน" AT ROW 4.24 COL 39.67
          BGCOLOR 1 FGCOLOR 7 FONT 6
     " As of Date":30 VIEW-AS TEXT
          SIZE 17.5 BY .95 TOOLTIP "วันที่ออกรายงาน" AT ROW 4.24 COL 3.17
          BGCOLOR 1 FGCOLOR 7 FONT 6
     "To":10 VIEW-AS TEXT
          SIZE 5 BY .95 TOOLTIP "ถึงสาขา" AT ROW 12.67 COL 7.5
          BGCOLOR 8 FGCOLOR 0 FONT 6
     " Include Type All":50 VIEW-AS TEXT
          SIZE 23.17 BY 1 AT ROW 10.76 COL 48.33
          BGCOLOR 1 FGCOLOR 7 FONT 6
     "To":20 VIEW-AS TEXT
     ...
     "Br. From":30 VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 0 FONT 6
     "                                                     STATEMENT A4 SUMMARY (Credit/Due)":100 VIEW-AS TEXT
...
DEFINE FRAME frST
     " Client Type Code":50 VIEW-AS TEXT
          SIZE 15 BY .76 TOOLTIP "Client Type Code" AT ROW 4.38 COL 76
          BGCOLOR 1 FGCOLOR 7 
     reBranch2 AT ROW 14.05 COL 2
     RECT-87 AT ROW 10.91 COL 2
     RECT-89 AT ROW 10.76 COL 48.83
     reReports2 AT ROW 2.81 COL 5.5
     reReprots AT ROW 2.29 COL 2
     reTrntyp1 AT ROW 11.86 COL 49
     RECT12 AT ROW 4.38 COL 76
     RECT-90 AT ROW 10.76 COL 78.83
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         NO-LABELS SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 4 ROW 1.52
         SIZE 108 BY 16.71
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "wacr031 - STATEMENT A4 SUMMARY (Credit/Due)"
         HEIGHT             = 24.05
         WIDTH              = 132.17
         MAX-HEIGHT         = 24.48
         MAX-WIDTH          = 133
...
/* SETTINGS FOR RADIO-SET ra_overdue IN FRAME frST
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* wacr031 - STATEMENT A4 SUMMARY (Credit/Due) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* wacr031 - STATEMENT A4 SUMMARY (Credit/Due) */
DO:
...
DEF VAR nv_filter1 AS CHAR INIT "".
   ...
   DO WITH FRAME {&FRAME-NAME} :
        ASSIGN
            ...
            FRAME frST seCliCod seCliCod  /*take*/
            ...
            vCliCod    = IF seCliCod:List-items = ? THEN vCliCodAll ELSE seCliCod:LIST-ITEMS /*take*/
...
ON VALUE-CHANGED OF ra_credit IN FRAME frST
DO:
   ra_credit = INPUT ra_credit .
   DISP ra_credit WITH FRAM frST.
...
&Scoped-define SELF-NAME ra_overdue
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ra_overdue C-Win
ON VALUE-CHANGED OF ra_overdue IN FRAME frST
DO:
  ra_overdue = INPUT ra_overdue .
  DISP ra_overdue WITH FRAM frST.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ra_select C-Win
ON VALUE-CHANGED OF ra_select IN FRAME frST
DO:
   ra_select = INPUT ra_select .
   DISP ra_select WITH FRAM frST.

    IF         ra_select = 1 THEN DO:
       ENABLE  ra_credit WITH FRAME {&FRAME-NAME}.
               ra_overdue = 0.
               DISP ra_overdue WITH FRAM frST.
               ra_credit = INPUT ra_credit .
       DISABLE ra_overdue WITH FRAME {&FRAME-NAME}.
    END.
    ELSE IF  ra_select = 2 THEN DO:
     ENABLE  ra_overdue WITH FRAME {&FRAME-NAME}.
             ra_credit = 0.
             DISP ra_credit WITH FRAM frST.
             ra_overdue = INPUT ra_overdue .
     DISABLE ra_credit WITH FRAME {&FRAME-NAME}.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY ra_select ra_credit toOut cbRptList fiagent fiagent2 fiTyp1 fiTyp2 
          fiTyp3 fiTyp4 fiTyp5 fiBranch fiBranch2 fiTyp6 fiTyp7 fiTyp8 fiTyp9 
          fiTyp10 fiTyp11 fiTyp12 fiTyp13 fiTyp14 fiTyp15 fiAsdat fiProcessDate 
          fibdes fiInclude fibdes2 finame1 finame2 fiProcess raReportTyp 
          seCliCod fiCcode1 ra_overdue 
      WITH FRAME frST IN WINDOW C-Win.
  ENABLE ra_select ra_credit toOut cbRptList brAcproc_fil buBranch fiagent 
         fiagent2 fiTyp1 fiTyp2 fiTyp3 fiTyp4 fiTyp5 fiBranch fiBranch2 
         buBranch2 fiTyp6 fiTyp7 fiTyp8 fiTyp9 fiTyp10 fiTyp11 fiTyp12 fiTyp13 
         fiTyp14 fiTyp15 fiAsdat fiProcessDate fibdes fiInclude fibdes2 finame1 
         finame2 fiProcess raReportTyp seCliCod fiCcode1 buClicod buAdd buDel 
         reBranch2 RECT-87 RECT-89 reReports2 reReprots reTrntyp1 RECT12 
         RECT-90 
      WITH FRAME frST IN WINDOW C-Win.
...
DEF VAR  n_CliCod  AS CHAR FORMAT "X(2)".
...
/********************** Page Header *********************/           
    OUTPUT TO VALUE (STRING(fiFile-Name)  ) NO-ECHO.
        IF vCount = 0 THEN DO:
            ...
            EXPORT DELIMITER ";"
                "ReportTyp : " + nv_ReportTyp.

            EXPORT DELIMITER ";"
                "Group Statement" "Client Type Code" 
                "Account No."  "Name"  "Credit"  "Credit Limit" "Gross" "Comm + Comm_comp" " Balance Due"
                "Within"  "Due Amount "  "Overdue"  
                /*--A54-0270---
                "1 - 3 months"  "3 - 6 months"  "6 - 9 months"  " 9 - 12 months"  "over 12 months". 
                ------------*/
                "1-30 days"   "31-60 days" "61-90 days" "91-365 days" "Over 365 days"  /*---A54-0270---*/
                "ชื่อผู้ติดต่อเก็บเงิน"  /*by eakkapong  A57-0240 */
                "เบอร์ติดต่อ" 
                "เบอร์โทรสาร" 
                "Email" .  
...
/********************** END Page Header *********************/  
    ...
    FOR EACH wtSum USE-INDEX wtSum1  :
            ...
            OUTPUT TO VALUE (STRING(fiFile-Name)  ) APPEND NO-ECHO.
                EXPORT DELIMITER ";"
                    wtsum.wtgpstmt
                    wtsum.clientco   /*by eakkapong  A57-0240 */
                    wtsum.wtacno
                    wtsum.wtacname
                    ...
                    wtsum.wtodue4
                    wtsum.wtodue5
                    wtsum.nv_abname   /*by eakkapong  A57-0240 */
                    wtsum.nv_tel      
                    wtsum.nv_Fax 
                    wtsum.nv_email .
...
DEF VAR nv_bdes AS CHAR.
    ...
    ELSE DO:
        nv_ReportTyp = "ALL ".
    END.
IF raReportTyp <> 1 AND  raReportTyp <> 2 AND  raReportTyp <> 3 THEN DO:     /* A47-0142   ดึงข้อมูล เฉพาะ บาง  client type code */
  ...
  RUN pdmDay.  /*A55-0231*/
    ...
    /********************** SUM DETAIL *********************/    
       ...
       /* TOTAL */
           ...
           nv_Credit1      = agtprm_fil.odue6 - nv_tot_bal  .
       /*---By A55-0231----
           nv_tot_odue1   = nv_tot_odue1 + agtprm_fil.odue1
           nv_tot_odue2   = nv_tot_odue2 + agtprm_fil.odue2
           nv_tot_odue3   = nv_tot_odue3 + agtprm_fil.odue3
  ...
  /*---end A55-0231---*/
    IF LAST-OF(agtprm_fil.acno)  THEN DO:  /**/
       ...
       IF NOT AVAIL wtSum THEN DO: 
          /*RUN pdfindcredit.*/
           IF ra_credit  = 3 OR ra_overdue = 3 THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
           IF ra_credit  = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
           IF ra_overdue = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue > 0  NO-LOCK NO-ERROR NO-WAIT.
           IF ra_credit  = 2  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  < 0  NO-LOCK NO-ERROR NO-WAIT.
           IF ra_overdue = 2  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue <= 0  NO-LOCK NO-ERROR NO-WAIT.
           
           IF AVAIL xmm600 THEN do: 
               nv_gpstmt = xmm600.gpstmt.    
               nv_clicod = xmm600.clicod.  /*by eakkapong  A57-0240 */
              ...
              ASSIGN
                  ...
                  wtsum.clientco  = nv_clicod.  

             nv_gpstmt = xmm600.gpstmt.
          IF agtprm_fil.acno <> nv_gpstmt THEN DO:
          
              FIND FIRST bxmm600 WHERE bxmm600.acno = nv_gpstmt  NO-LOCK NO-ERROR.
              IF AVAIL bxmm600 THEN DO:
                         nv_abname = bxmm600.NAME .
                         nv_tel    = bxmm600.phone.
                         nv_fax    = bxmm600.fax.
               END.
               ELSE DO:
                 ASSIGN  nv_abname = ""
...
FOR EACH wtSum : DELETE wtsum. END.
    FOR EACH  agtprm_fil USE-INDEX by_acno            WHERE
        ...
        DISP  /*agtprm_fil.acno --- A500178 ---*/
              ...
              agtprm_fil.trntyp
              agtprm_fil.docno
        WITH COLOR BLACK/WHITE NO-LABEL FRAME frProcess4 VIEW-AS DIALOG-BOX
        TITLE  "  Processing ...".
        ...
        RUN pdmDay.  /*A55-0231*/
            ...
            /*================== เปรียบเทียบวันที่ As Date กับ duedat & odat1-4 (over due date) ===*/
               ...
               END.
               IF n_asdat > n_odat4 THEN DO:
                       n_odue5 = n_odue5 +  agtprm_fil.bal.        /*over 365 days */   /* เดิม over 12  months*/END.
                   ...
                   /* RUN pdfindcredit.  */ 
                    ...
                    IF ra_overdue = 2  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue <= 0  NO-LOCK NO-ERROR NO-WAIT.
                       ...
                       ASSIGN
                           ...
                           wtsum.wtcomm    = nv_tot_comm
                           wtsum.wtcomm_comp = nv_tot_comm_comp
                           wtsum.clientco  = nv_clicod.  /*by eakkapong  A57-0240 */
    ...
    OUTPUT TO VALUE (STRING(fiFile-Name)  ) NO-ECHO.
        IF vCount = 0 THEN DO:
            ...
            IF nv_out = FALSE  THEN DO:
                ...
                EXPORT DELIMITER ";"
                    "Agent"
                    "Account No." "Client Type Code" "Name"  "Credit"  "Credit Limit"  "Group Statement"  "Gross" "Comm + Comm_comp"  " Balance Due"
                    "Within"  "Due Amount "  "Overdue"  
                    /*---By A54-0270--
                    "1 - 3 months"  "3 - 6 months"  "6 - 9 months"  " 9 - 12 months"  "over 12 months". 
                    ------------------*/
            ...
            /********************** SUM DETAIL *********************/    
             ...
             IF LAST-OF(agtprm_fil.agent)  THEN DO:  /**/
                ASSIGN
                    nv_gpstmt = ""
                    nv_gpname = "".
                   FIND FIRST wtSum USE-INDEX wtSum4 WHERE  wtsum.wtagent = agtprm_fil.agent NO-ERROR.
                  IF ra_credit  = 3 OR ra_overdue = 3 THEN FIND FIRST wtSum USE-INDEX wtSum4 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
                  IF ra_credit  = 1  THEN FIND FIRST wtSum USE-INDEX wtSum4 WHERE (wtsum.wtagent = agtprm_fil.agent)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
                  IF ra_overdue = 1  THEN FIND FIRST wtSum USE-INDEX wtSum4 WHERE (wtsum.wtagent = agtprm_fil.agent)  AND nv_tot_odue <> 0  NO-LOCK NO-ERROR NO-WAIT.
                  IF ra_credit  = 2  THEN FIND FIRST wtSum USE-INDEX wtSum4 WHERE (wtsum.wtagent = agtprm_fil.agent)  AND nv_Credit1  < 0  NO-LOCK NO-ERROR NO-WAIT.
                  IF ra_overdue = 2  THEN FIND FIRST wtSum USE-INDEX wtSum4 WHERE (wtsum.wtagent = agtprm_fil.agent)  AND nv_tot_odue = 0  NO-LOCK NO-ERROR NO-WAIT.
                ...
                IF NOT AVAIL wtSum THEN DO:
                    ...
                    FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.agent) NO-LOCK NO-ERROR NO-WAIT.
                    IF AVAIL xmm600 THEN  nv_gpstmt = xmm600.gpstmt.
                    ELSE nv_gpstmt = "".

                    ASSIGN wtsum.wtgpstmt  = nv_gpstmt.
  ...
  /*---end A55-0231---*/
    IF LAST-OF(agtprm_fil.acno)  THEN DO:  /**/
       ASSIGN
           nv_gpstmt = ""
           nv_gpname = "".
       FIND FIRST wtSum USE-INDEX wtSum4 WHERE  wtsum.wtagent = agtprm_fil.agent AND wtsum.wtacno  = agtprm_fil.acno NO-ERROR.     /* A47-0500 */
     IF NOT AVAIL wtSum THEN DO:
          IF ra_credit  = 3 OR ra_overdue = 3 THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
          IF ra_credit  = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
          IF ra_overdue = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue > 0  NO-LOCK NO-ERROR NO-WAIT.
        ...
        /*---end A55-0231---*/
          IF LAST-OF(agtprm_fil.acno)  THEN DO:  /**/
             ...
             FIND FIRST wtSum USE-INDEX wtSum4 
                              WHERE  wtsum.wtagent = agtprm_fil.agent AND wtsum.wtacno  = agtprm_fil.acno NO-ERROR.     /* A47-0500 */
             IF NOT AVAIL wtSum THEN DO:
                 IF ra_credit  = 3  OR ra_overdue = 3 THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
                 IF ra_credit  = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
                 IF ra_overdue = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue > 0  NO-LOCK NO-ERROR NO-WAIT.
           ...
           /*---end A55-0231---*/
             IF LAST-OF(agtprm_fil.acno)  THEN DO:  
                ASSIGN
                    nv_gpstmt = ""
                    nv_gpname = "".
                FIND FIRST wtSum USE-INDEX wtSum1 WHERE wtsum.wtacno = agtprm_fil.acno AND wtsum.wtbran  = agtprm_fil.polbran NO-ERROR.
                IF NOT AVAIL wtSum THEN DO:
                    IF ra_credit  = 3  OR  ra_overdue = 3 THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
                    IF ra_credit  = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
                    IF ra_overdue = 1  THEN FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue > 0  NO-LOCK NO-ERROR NO-WAIT.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pdfindcredit C-Win 
PROCEDURE pdfindcredit :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
 ...
 IF ra_credit  = 3 AND ra_overdue = 3 THEN DO:
    FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno) NO-LOCK NO-ERROR NO-WAIT.
    IF AVAIL xmm600 THEN do: 
               nv_gpstmt = xmm600.gpstmt.    
               nv_clicod = xmm600.clicod.  /*by eakkapong  A57-0240 */
 ...
 IF ra_credit  = 1  THEN  DO:
     ...
     FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_Credit1  >= 0  NO-LOCK NO-ERROR NO-WAIT.
     MESSAGE  nv_Credit1  VIEW-AS ALERT-BOX INFORMATION.
     IF AVAIL xmm600 THEN do: 
               nv_gpstmt = xmm600.gpstmt.    
               nv_clicod = xmm600.clicod.  /*by eakkapong  A57-0240 */
              CREATE wtSum.
 ...
 IF ra_overdue = 1  THEN DO:
      FIND FIRST xmm600 USE-INDEX xmm60001 WHERE (xmm600.acno = agtprm_fil.acno)  AND nv_tot_odue >= 0  NO-LOCK NO-ERROR NO-WAIT.
      IF AVAIL xmm600 THEN do: 
               nv_gpstmt = xmm600.gpstmt.    
               nv_clicod = xmm600.clicod.  /*by eakkapong  A57-0240 */
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:     Add by  A55-0231  Sayamol N. 17/07/2012
------------------------------------------------------------------------------*/

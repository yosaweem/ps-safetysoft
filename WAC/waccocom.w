...
/************   Program   **************
/* CREATE BY :  Kanchana C.     A47-0406    13/10/2004  */
...
- ระบุ trndat from-to ของข้อมูล




/* MODIFY BY : Shukiat   Assign No.  :   A48-0177  */
Q1-48 เบี้ยประกันภัยค้างรับ ส่งกรม 
...
DEFINE IMAGE IMAGE-22
     FILENAME "wimage/backgrou.bmp":U CONVERT-3D-COLORS
     SIZE 126 BY 23.76.
...
DEFINE FRAME frST
     ...
     "Branch From":30 VIEW-AS TEXT
          ...
          BGCOLOR 8 FGCOLOR 0 
     "                                                    Ageing Count Day By Comdate":210 VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Waccocom - Ageing Count Day By Comdate"
         HEIGHT             = 23.57
         WIDTH              = 131.67
         MAX-HEIGHT         = 31.33
         MAX-WIDTH          = 170.67
...
IF NOT C-Win:LOAD-ICON("I:/Safety/W_TEST9/WIMAGE/safety.ico":U) THEN
    MESSAGE "Unable to load icon: I:/Safety/W_TEST9/WIMAGE/safety.ico"
...
ON END-ERROR OF C-Win /* Waccocom - Ageing Count Day By Comdate */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Waccocom - Ageing Count Day By Comdate */
DO:
...
/*             nv_prninslog = toprnIns */
   ...
   END.
    ...
    IF nv_RptName = "Agent" THEN DO:  
        IF  n_frac = "" THEN  n_frac = "".
        IF  n_toac = "" THEN  n_toac = "ZZZZZZZZZZ". 
    END.
    ELSE DO:
        IF  n_frac = "" THEN  n_frac = "A000000".
...
/********************  T I T L E   F O R  C - W I N  ****************/
  ...
  gv_prgid = "Waccocom".
  gv_prog  = "Ageing Count Day By Comdate".
 ...
 *   RUN ProcGetPrtList.  */
        ...
        ASSIGN   
            ...
            raReport:Radio-Buttons = "With Credit Term, 1,Comm Date, 2"
            raReport = 1
...
------------------------------------------------------------------------------*/
  ...
  ENABLE fiBranch fiBranch2 fiPolTypFr fiPolTypTo fiacno1 fiacno2 fiasdatAging 
         ...
         fibdes fibdes2 fiPoldesFr fiPoldesTo finame1 finame2 fiInclude fiAsdat 
         fiProcessDate fiProcess re1 re11 re12 re13 re14 RECT-302 
      WITH FRAME frST IN WINDOW C-Win.
...
FOR EACH wtGrpSum:  DELETE wtGrpSum.  END.
    ...
    /*RUN pdForGrpCode. /* หา Group code */*/
...
/*** DISPLAY SUMMARY FILE & SUMMARY GRAND TOTAL***/
    ...
    FOR EACH   agtprm_fil WHERE /* use-index by_acno */
...
---*/
                ...
                EXPORT DELIMITER ";"  wtGTrptname + " Premium"          wtGTTprem   wtGTprem[1 FOR 4]. 
                EXPORT DELIMITER ";"  wtGTrptname + " Commission"       wtGTTcomm   wtGTcomm[1 FOR 4].
                EXPORT DELIMITER ";"  wtGTrptname + " Bal. O/S"         wtGTTbal    wtGTbal[1 FOR 4].
                EXPORT DELIMITER ";" wtGTrptname + " Cheque Returned"   wtGTTretc   wtGTretc[1 FOR 4].
                EXPORT DELIMITER ";"  wtGTrptname + " Suspense"         wtGTTsusp   wtGTsusp[1 FOR 4].
                EXPORT DELIMITER ";"  wtGTrptname + " Net. A/R"         wtGTTnetar  wtGTnetar[1 FOR 4].
                EXPORT DELIMITER ";"  wtGTrptname + " Net. OTHER"       wtGTTnetoth wtGTnetoth[1 FOR 4].

            
                EXPORT DELIMITER ";" "Summary of : " + wtGTrptname + " " + wtGTacno
                    "ไม่เกิน 60 วัน" + STRING(wtGTbal[1] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "61 - 90 วัน" + STRING(wtGTbal[2] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "91 - 365 วัน = " + STRING(wtGTbal[3] ,">>,>>>,>>>,>>9.99-") + nv-1 +
                    "365 วันขึ้นไป = " + STRING(wtGTbal[4] ,">>,>>>,>>>,>>9.99-").
...
/*------------------------------------------------------------------------------
  Purpose: Ageing Detail  By Agent   /* A48-0265 */
------------------------------------------------------------------------------*/
...
/*** DISPLAY SUMMARY FILE & SUMMARY GRAND TOTAL***/
    ...
    FOR EACH   agtprm_fil WHERE /* use-index by_agent */
               agtprm_fil.asdat        = n_asdat     AND
    ...
    NO-LOCK BREAK  BY agtprm_fil.agent 
             ...
             DISP agtprm_fil.agent  FORMAT "X(10)" /*--- A500178 --- เพิ่ม format "X(10)"---*/
...
/*** DISPLAY SUMMARY FILE & SUMMARY GRAND TOTAL***/
    ...
    NO-LOCK BREAK BY SUBSTRING(agtprm_fil.policy,3,2)
                  BY agtprm_fil.agent
                  BY agtprm_fil.comdat /*note change from comdat*/
                  BY agtprm_fil.policy.

        DISP  SUBSTRING(agtprm_fil.policy,3,2) FORMAT "X(5)" /*-- A500178 -- เพิ่ม FORMAT --*/
              agtprm_fil.agent FORMAT "X(10)" /*-- A500178 -- เพิ่ม FORMAT --*/
/*               agtprm_fil.acno */
              agtprm_fil.trndat
              agtprm_fil.policy
              agtprm_fil.trntyp
...
IF nv_RptName = "Line" THEN DO:
    EXPORT DELIMITER ";"
        "Agent No."  "Agent Name  " "Branch" "Credit Term"  "Tran Date" "Due Date" " Invoice No."  " Policy No."  " Endt No."  " Com. Date " "Exp date" /*" Insured Name "*/  /*note add  expi*/
...
ELSE DO:
    EXPORT DELIMITER ";"
        "Account No."  "Account Name  " "Branch" "Credit Term"  "Tran Date" "Due Date" " Invoice No."  " Policy No."  " Endt No."  " Com. Date " "Exp date" /*" Insured Name "*/  
    /*    "Premium  "  " Compulsory"  " Stamp"  "Tax"  "Total"  "Comm."  " Comm_comp" "Net amount"  "Balance (Baht)"*/

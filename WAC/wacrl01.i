...
/*            ladat = IF  LOOKUP(n_trnty1,nv_MR) <> 0 THEN agtprm_fil.duedat   ELSE agtprm_fil.trndat.  /* เฉพาะ type MR เท่านั้นที่ต้องคิด duedate  */*/
          ...
          /* พี่ปรีไม่ใช้ With Credit Term - แจ้งให้เอาออกได้เลย*/
          /*--- IF nv_RptName2 = "With Credit Term" THEN DO: ---By A54-0069---*/
            IF nv_RptName2 = "By Transaction Date" THEN DO:   /*A54-0069*/
                /*---A54-0069---
                ladat   = IF  LOOKUP(n_trnty1,nv_MR) <> 0 THEN agtprm_fil.duedat  
                          ELSE agtprm_fil.trndat.  /* เฉพาะ type MR เท่านั้นที่ต้องคิด duedate  */
                ---------------*/
                 ladat   = agtprm_fil.trndat.     /*---A54-0069---*/
                ...
                FIND FIRST uwm100 USE-INDEX uwm10090 WHERE 
                           ...
                           uwm100.policy = agtprm_fil.policy NO-ERROR.
                IF AVAIL uwm100 THEN DO:
                    IF uwm100.endcnt > 000 THEN ASSIGN ladat = uwm100.enddat
                                                       n_enddat = STRING(uwm100.enddat).
                    ELSE ASSIGN ladat = uwm100.trndat
                                n_enddat = "".
                END.

                /*---IF ladat = ? THEN ladat = agtprm_fil.comdat.---A54-0069---*/
                IF ladat = ? THEN ladat = agtprm_fil.trndat.
            END.
            ELSE DO:
               /*---A53-0159---
               ...
               /*----Policy--- ใช้ Comdat ถ้าไม่มีใช้ trndat ---*/
               ELSE DO:   /*Policy*/
                   ASSIGN n_enddat = ""
                          ladat  = agtprm_fil.comdat.
               END.
               /*------end A53-0159------*/
            ...
            /***---
            /***--- change ladat เปลี่ยนเป็นวันที่ 1 ของเดือนถัดไป ---***/
            ...
            IF n_asdat > ladat THEN DO:  /*A50-0052 Shukiat T. Modi ladat1 --> ladat */
                qdat = (n_asdat - ladat) + 1.  /*A50-0052 Shukiat T. Modi ladat1 --> ladat */
                /*----
                IF qdat > 365  THEN
                    lip = 4.
                ...
                /*---A54-0119---
                /*---A53-0159 Sayamol---*/
                IF qdat > 365      THEN  lip = 9.
                ELSE IF qdat > 270 THEN  lip =  8.
                ELSE IF qdat > 180 THEN  lip =  7.
                ELSE IF qdat > 90  THEN  lip =  6.
                ELSE IF qdat > 60  THEN  lip =  5.
                ELSE IF qdat > 45  THEN  lip =  4.
                ELSE IF qdat > 30  THEN  lip =  3.
                ELSE IF qdat > 15  THEN  lip =  2.
                ELSE lip = 1. 
                /*---end A53-0159 Sayamol---*/
                end A54-0119---*/

                /*---A54-0119 Sayamol---*/
                IF qdat > 365      THEN  lip = 15.
                ELSE IF qdat > 330 THEN  lip = 14.
                ELSE IF qdat > 300 THEN  lip = 13.
                ELSE IF qdat > 270 THEN  lip = 12.
                ELSE IF qdat > 240 THEN  lip = 11.
                ELSE IF qdat > 210 THEN  lip = 10.
                ELSE IF qdat > 180 THEN  lip = 9.
                ELSE IF qdat > 150 THEN  lip = 8.
                ELSE IF qdat > 120 THEN  lip = 7.
                ELSE IF qdat > 90  THEN  lip = 6.
                ELSE IF qdat > 60  THEN  lip = 5.
                ELSE IF qdat > 45  THEN  lip = 4.
                ELSE IF qdat > 30  THEN  lip = 3.
    ...
    IF (nv_DetSum = "Detail")  OR (nv_DetSum = "All") THEN DO:
        ...
        /* end A47-0264 */
         ...
         FIND FIRST uwm100 USE-INDEX uwm10001 WHERE 
                     ...
                     uwm100.endcnt = acm001.endcnt  NO-ERROR.
         IF AVAIL uwm100 THEN DO:
           IF uwm100.endcnt > 000 THEN ASSIGN n_enddat = STRING(uwm100.enddat).
           ELSE ASSIGN n_enddat = "".
         END.
         ...
         IF AVAIL uwm100 THEN DO:
            IF uwm100.endcnt > 000 THEN n_enddat = STRING(uwm100.enddat).
            ELSE n_enddat = "". 
         END.
         ELSE DO:
            FIND FIRST uwm100 USE-INDEX uwm10090 WHERE 
        ...
        IF nv_RptName = "Line" THEN DO: /* A48-0265 */
            ...
            OUTPUT TO VALUE (STRING(n_OutputFile ) ) APPEND NO-ECHO.
                EXPORT DELIMITER ";"
                    ...
                    /* nv_balDet[1 FOR 13] /*---Lukkana M. A52-0318 26/11/2009---*/---A53-0159---*/
                    /*--- A54-0119  
                    nv_balDet[1 FOR 9]   /*A53-0159*/  
                    ---*/
                    nv_balDet[1 FOR 15]  /*--A54-0119--*/
                    nv_polgrp
                    nv_insref            /*A53-0159*/
                    .
            OUTPUT CLOSE.
